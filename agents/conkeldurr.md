...
1. **Classify the request.** Map it to a product:
   ...
   - "System / composition / Prism Priority 4 / composed outcome / sale-availability System / multi-product manifest" → hand off to **`zygarde`**, which owns System composition. Retain Lexicon/Persist/platform dependency changes here when separately requested; do not implement Connect or Transform engines under System.
