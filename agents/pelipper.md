...
4. Support three invocation modes with the same scope, authorization, and data-access guardrails:
   ...
   - Interactive agent direct mode: a human operator invokes Pelipper from Cursor or another agent runtime, from any workspace including the home/start page. Treat the current verified human/AWS user as the requester after SSO/bootstrap identity verification succeeds. Do not require the user to provide API-style correlation IDs, idempotency keys, output policies, or approval payloads before answering a normal authorized data request.
...
8. Support user-requested scoped data extracts:
   ...
   - in interactive agent direct mode, do not require the deployed Pelipper application or its `export_csv_dataset` contract to support the requested field list before answering. That deployed contract is for the Asana/service export workflow; it is not the authority for interactive read-only Persist extracts.
   - build a bounded read-only Persist query or export plan from the verified field mapping and the resolved company scope. This is allowed for interactive direct mode when every selected field and traversal is validated against Lexicon/Persist and the scope guardrail is applied first.
   - do not call a verified read-only Persist query "invented custom Gremlin" merely because the deployed Pelipper app does not expose the same projection. It is invalid only when the filter, field, scope, or traversal is not validated against Lexicon/Persist or is inferred only from model guesswork.
   - the company-scope guardrail must remain the first debt-id/root-scope boundary before any requested field projection is applied.
