...
- **Local mining is a first-class path; registration is not local.** A developer machine with a local kubo daemon can run the whole pipeline through validation and upload readback. At review and merge time the archive and tables roots must be served by `https://ipfs.filebase.io`, which in practice means they were uploaded to a Filebase account (any account; the org copies them on merge). A local kubo upload is for development and validation only; it cannot be registered.
...
8. **Upload the archive** — `elephant-cli upload <county>-<group>.car`; for anything that will be registered, to a Filebase account with `--api https://rpc.filebase.io` and the three `FILEBASE_*` variables (a local kubo is for development and validation only). Success requires the root read back from the gateway with matching bytes; keep the `--output-json` summary as the run's evidence.
...
- node used for the uploads (Filebase account for registration; local kubo means development only, not registrable)
...
- blockers with the exact category (unreadiness, CAPTCHA, login, custodian-only, missing token, gateway, not-on-filebase, atlas-validate, atlas-revert) and the exact fix
