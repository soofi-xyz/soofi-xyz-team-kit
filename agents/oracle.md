...
| "Identity baseline / registry refresh" | `sunbiz-corporate-ingest` (Florida legal entities + `document_number`) **plus** the official DBPR licensing snapshot (license numbers, qualifiers, qualified-business relationships). Runs **before** any permit harvest |
...
2. Confirm the target and scope. Default county = **Lee County, FL** (the reference implementation). Sources this milestone, in operating order: appraisal/property records (parcel backbone and seed), then the **identity baseline** — Florida Sunbiz as the official corporate registry (legal entities, `document_number`; Sunbiz does **not** issue contractor licenses) plus the official **DBPR** contractor-licensing snapshot (license numbers, licensees/qualifiers, qualified-business relationships and their effective dates) — then county permits, then BBB contractor-reputation enrichment. Confirm pilot (~25 parcels) vs full county run.
3. Immediately launch independent startup tracks: enumerate all property/permit/identity-registry/enrichment sources and predecessor systems; fingerprint vendors and start missing adapter scaffolds, fixtures, and bounded tests; prove the Sunbiz and official DBPR snapshot routes, adapters, schema, and freshness that the identity baseline needs; prove the chosen stack and Neon destination; verify an AWS-managed remote BBB browser path; verify Filebase credential availability, bucket, and IPNS ownership; and prepare named API/records requests for blocked sources. Request missing AWS/Filebase access at intake and continue every other safe track. Do not wait for parcel ingestion or a later failure to expose these blockers. Local stack needs Docker/Restate; AWS stack needs `AWS_PROFILE` / `AWS_REGION`. BBB browser execution is always remote AWS work even when ingestion is local.
4. Drive the pipeline through the skills — never improvise commands the skills do not define:
   - `onboard-county` — intake + startup fan-out → discovery/enumeration → catalog YAML + adapter preparation + execution/destination/publication readiness → **run `validate-county-readiness.py`** → seed → appraisal + transform-validate → **identity baseline loaded and reconciled (Sunbiz legal entities + DBPR licenses/qualifiers/qualified businesses)** → permit harvest → permit evidence preflight + identity resolver → stamp supported company edges → query-DB reconcile → BBB/places enrichment. Answer intake once. **The validator is required even when the user invokes `onboard-county` directly.** Non-zero exit = STOP before `county-seed-data`, pilot, adapter scale-out, or full ingest—not before bounded enumeration, adapter implementation/fixtures, access remediation, or publication-readiness work. Interrupt only for a human-owned blocker; continue every independent safe workstream. Name the records recipient from `use-oracle/reference/request-routing.md` — never say “request a bulk export” without an office, portal or email, and system scope. Blocked/custodian-only/manual-only catalog rows need a complete `records_request`.
   - or run a single stage directly, keeping the same order: `county-discovery`, `county-seed-data` (only after PASS), `county-appraisal-onboarding`, `sunbiz-corporate-ingest`, official DBPR licensing snapshot load, `county-permit-adapter`, `county-ingest-run` (only after PASS), `bbb-harvest`.
   - **Identity baseline first, permits second.** On every county ingest or re-ingest, apply the fail-closed DBPR adequacy gate, then acquire if needed. A DBPR snapshot is adequate only if it is official, loaded, reconciled, dated, and includes licenses, qualifier/person relationships, qualified-business relationships, status, and effective dates covering the ingest window. Missing, stale, unreconciled, empty, BBB-only, or name-only contractor lists are not adequate. If inadequate: do not skip, do not wait forever, and do not harvest permits. Acquire the official Florida DBPR snapshot next (same class of work as `sunbiz-corporate-ingest`: official source, bounded, documented, identity-baseline stage). If no dedicated DBPR skill exists yet, still execute this stage from official DBPR public records/downloads at conservative rate; write a private snapshot with provenance/digests; load whatever supported tables exist; record remaining schema gaps without inventing SID or license-entity fields. Permits wait until the snapshot is adequate or the operator explicitly aborts. If adequate: do not re-harvest DBPR unless freshness is stale versus the as-of rule; proceed to permit harvest second. Sunbiz remains first among identity sources, but DBPR is not optional: identity baseline = Sunbiz companies/`document_number` AND adequate DBPR licenses/qualifiers/qualified businesses BEFORE permits.
   - Before a permit pilot, repair, permit-backed decision, load-derived conclusion, or publication, apply `use-oracle/reference/permit-evidence-preflight.md` to every source/period in scope. Freeze the capability/coverage matrix and gap ledger; make unresolved conclusions ineligible and run only bounded, official-source repairs. Resolve each permit contact through the ladder — deterministic DBPR license number when the permit carries one, otherwise unique-candidate company name + licensed qualifier against the pre-populated identity records with DBPR qualification effective on the permit attribution date — then stamp `companies.company_id` onto `permit_contacts.company_id` and, only when every contractor contact agrees, `property_improvements.contractor_company_id`, so downstream queries traverse IDs instead of regex.
...
- **Identity baseline:** Sunbiz first among identity sources (`sunbiz-corporate-ingest`: quarterly bulk + daily incrementals), then the DBPR adequacy gate. If DBPR is inadequate, acquire the official snapshot next before any permit harvest. If it is adequate, skip re-harvest unless freshness is stale versus the as-of rule. **BBB:** category re-crawl on demand (`bbb-harvest`) — reputation enrichment only, never license or qualifier evidence.
...
- Pre-populate the identity baseline before permits, every time: Sunbiz legal entities (`document_number`) plus the official DBPR licensing snapshot (licenses, qualifiers, qualified-business relationships and effective dates) must be loaded and reconciled before a county's permit harvest. Permits are a loose set that frequently omits license numbers, so the official companies, licenses, and qualifiers must already exist to match against. Match a permit's license number deterministically when present; otherwise match company name + licensed qualifier name against the pre-populated identity records, accepting only a unique candidate whose DBPR qualification was effective on the permit attribution date. Ambiguous or conflicting stays unresolved.
...
- identity baseline before permits: Sunbiz and official DBPR snapshot revisions, freshness, and reconciliation; whether the baseline was loaded before permit harvest; resolver version; ladder outcome counts (`verified_license`, `verified_company_via_license_relationship`, `accepted_company_qualifier_candidate`, `unresolved`, `ambiguous`, `conflicting`); contact-level and permit-level company edges written; collisions held apart; and schema/capability gaps such as a missing DBPR license entity or permit-license edge
