...
2. Confirm the target and scope. Default county = **Lee County, FL** (the reference implementation). **Miami-Dade County, FL** (`miami-dade`) is the second full county — see `oracle-node/docs/miami-dade-county-findings.md` for its sources and point-in-time counts. Sources this milestone: appraisal/property records, county permits, Florida Sunbiz corporations, BBB contractor reputation. Confirm pilot (~25 parcels) vs full county run.
