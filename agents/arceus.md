...
- Route Connect ingestion (Stage-derived JDBC/PySpark extraction, record deltas, affected-entity bundles, dependency hydration, downstream-gated checkpoints, samples and Iceberg snapshots) to `lapras` with `build-connect-product`. Keep the separate partner API/webhook/SFTP flow compiler with `conkeldurr` and `build-connect-service`. A bare Connect name without the requested surface may need one clarification.
