...
1. Route current report counts and report data questions to an approved live production source:
   ...
   - Discover live Athena workgroups, result settings, Glue databases/tables/columns/partitions, entity identifiers, and timestamps before querying. Planned communication tables or intended January 2026 history are not evidence that data exists. The derived-index snapshot date is fixed until a newer extract is published.
...
1. **Collect local project and AWS access.**
   ...
   - For approved communication/call counts and date filters, discover Athena/Glue through the canonical access skill. For ACTIVE derived-index snapshot questions, discover `debt_derived_indexes.active_index_values` via Glue/`DESCRIBE` and state snapshot date `2026-07-20`. Otherwise use Persist/Rules as the report source of truth and discover those deployed access paths from approved configuration.

2. **Internally classify source and mutation intent.**
   - Athena-approved read: the request is a count or easy date filter over orchestrated call eligibility/schedules or a live discoverable phone-call, email-message, or SMS-message entity, **or** a debt-level lookup/aggregation answered by ACTIVE Persist derived-index columns in `debt_derived_indexes.active_index_values`. Inspect live metadata first; do not route merely because a planned entity is named.
...
7. **Build and test optimized source queries before comparing results.**
   ...
   - State the actual available entities, date ranges, and gaps before answering. The `orchestrate_call_outputs` backfill warning is narrow: its current run-aware tables are static through `2026-07-14` with no automatic refresh; do not apply that warning to unrelated or future Athena tables. The derived-index table is a point-in-time snapshot dated `2026-07-20`.
...
Production Athena also exposes a point-in-time snapshot of ACTIVE Lexicon derived-index values for debts. This is an approved Athena data source alongside communication/call outputs.
...
- Local refresh/query code can regenerate report artifacts from approved Athena queries (communication/call and/or `debt_derived_indexes.active_index_values`), Persist in the selected environment, exact direct Persist filters/rules, and/or the latest Rules-released output with the verified AWS profile.
