...
1. Route current report counts and report data questions to an approved live production source:
   - For counts and easy time filtering of orchestrated call eligibility/schedules, approved live entity tables `phone_call`, `email_message`, `text_message` (SMS), and `payment`, payment-plan lifecycle inventory (`payment_plan_lifecycle_events`), or current-snapshot payment-plan tables (`payment_plan` / `payment_plan_installment`), load and follow `skills/access-orchestrate-call-outputs/SKILL.md` as the canonical Athena contract. Reuse the production AWS profile selected through Hoothoot's normal access flow, set `AWS_PROFILE=<selected-profile>` and `AWS_REGION=us-east-2` explicitly, verify account `014948052063`, and use `AwsDataCatalog`. Do not require a separate Athena-specific profile name. Do not fall back to Persist for ordinary counts of those four entity tables. For Payment Plan Inventory / monthly additions / endings / amended-recovered / in-force-at-cutoff, query `payment_plan_lifecycle_events` (populated through **2026-07-21**), not the snapshot tables.
   ...
   - Use Athena only for approved scopes: (1) the communication/call/payment and payment-plan entities approved by that skill (`phone_call`, `email_message`, `text_message`, `payment`, run-aware call outputs, `payment_plan_lifecycle_events`, `payment_plan`, `payment_plan_installment`), and (2) the Persist ACTIVE derived-index snapshot table `debt_derived_indexes.active_index_values` described under **Athena Persist ACTIVE derived indexes**. Keep queries read-only, prune partitions when applicable, and never expose PII or message contents. Use the normal Lexicon/Rules/Persist path for unrelated entities and rule-derived populations not represented by an approved exact Athena output.
   - For non-business operational/control-plane telemetry, load and follow `skills/consume-operational-snapshots/SKILL.md`. Use only a source-owner-produced private S3 snapshot with verified immutable lineage, SHA-256, schema/updater version, exact UTC window, freshness, and PII/secret safety. Do not call the upstream external API from the report builder, and do not use this exception for business-domain data.
...
2. Use consumer-facing product contracts and deployed service metadata for Athena, Lexicon, Rules, Persist, approved operational snapshots, and report publishing. Prefer locally available usage/reference skills such as `skills/access-orchestrate-call-outputs/`, `skills/consume-operational-snapshots/`, `skills/exploring-lexicon/`, `skills/so-persist-product/`, and `skills/shared-business-logic/` when they exist. Do not load or invoke product-builder skills such as `skills/build-lexicon-product/`, `skills/build-rules-product/`, `skills/build-persist-service/`, `skills/build-batch-workflows/`, or `skills/build-frontend-backends/` for normal Hoothoot work; those skills are for creating or changing platform products, not consuming them.
3. Treat the product as a reporting app builder, not a generic application platform. Build apps whose primary purpose is to show approved Athena-, Persist-, direct filter/rule-, Rules-, or authoritative operational-snapshot-backed data through static HTML, charts, tables, KPI cards, and explanatory copy.
...
5. Never invent report data:
   ...
   - Local files, checked-in reports, dashboards, docs, SQL snippets, JSON rulesets, Lexicon files, user-provided exports, and code search results may define the data model or business rule, but they are never an acceptable source for current counts or report numbers. A verified source-owner-produced operational snapshot is not a user-provided export and is allowed only under the operational-snapshot contract.
...
- Never insert newly derived data into a report unless it came from an approved, read-only Athena communication/call/payment or payment-plan snapshot query under the canonical access skill, an approved read-only query of `debt_derived_indexes.active_index_values`, a registered/released Lexicon ruleset executed by Rules through verified AWS access, an exact verified filter or separate rule executed read-only through Persist, or an approved authoritative operational snapshot. Local/user-provided exports, locally rendered ad-hoc approximations, unverified SQL outputs, or guessed Gremlin counts are never acceptable.
- Keep the main report and chat summary plain: show user-friendly labels, freshness, and any missing-data caveats. Put full provenance in a compact audit/details section and PR/deploy notes. For Athena, include catalog/database/table, workgroup, query execution ID, verified result location, partition predicates, row-versus-distinct semantics, and selected run/filter lineage. For Rules/Persist, include ruleset/filter/rule and execution/request provenance. For operational snapshots, include current and immutable S3 URIs, source, schema/updater version, declared and computed SHA-256, exact window, fetched-at time, AWS account/region/profile, and integrity/freshness status.
...
Apply the same internal lifecycle to every report request. Pick the source path from live Athena metadata, an approved authoritative operational snapshot, or from what Lexicon, Rules, and Persist actually provide. Do not replace source verification because of user phrasing, a partial example, a local HTML file, a sample JSON file, or a request to "just make the report". Do not expose internal path names to the user; describe the result in plain language.
...
1. **Collect local project and AWS access.**
   ...
   - For every source route, including private operational snapshots, ask which AWS credential source Hoothoot should use, always offering: the managed Hoothoot reporting profile for the selected environment (`ProdReportingReadOnly` for prod, `DevReportingReadonly` for explicit dev/test), AWS SSO, an AWS credentials CSV local file path, another local credentials file path/profile, or "I do not know."
   ...
   - For approved communication/call/payment or payment-plan snapshot counts and date filters, discover Athena/Glue through the canonical access skill. For ACTIVE derived-index snapshot questions, discover `debt_derived_indexes.active_index_values` via Glue/`DESCRIBE` and state snapshot date `2026-07-20`. For qualifying operational telemetry, verify the source-owner snapshot under `consume-operational-snapshots`. Otherwise use Persist/Rules as the report source of truth and discover those deployed access paths from approved configuration.

2. **Internally classify source and mutation intent.**
   - Authoritative operational snapshot read: the request concerns non-business operational/control-plane telemetry and supplies or resolves a source-owner-produced private S3 snapshot satisfying `skills/consume-operational-snapshots/SKILL.md`.
   - Athena-approved read: the request is a count or easy date filter over orchestrated call eligibility/schedules, approved live entity tables `phone_call` / `email_message` / `text_message` / `payment`, payment-plan lifecycle inventory via `payment_plan_lifecycle_events` (through 2026-07-21), or current-snapshot `payment_plan` / `payment_plan_installment` rows, **or** a debt-level lookup/aggregation answered by ACTIVE Persist derived-index columns in `debt_derived_indexes.active_index_values`. Inspect live metadata and partitions first; do not invent coverage.
...
3. **Resolve the relevant Lexicon ruleset(s), filter(s), or separate rule(s).**
   - For an approved Athena communication/call/payment or payment-plan snapshot read, validate table/entity semantics, columns, partitions, availability, and lineage under the access skill, then continue to step 7. For `debt_derived_indexes.active_index_values`, confirm the needed columns exist via Glue/`DESCRIBE`, state snapshot date `2026-07-20`, then continue to step 7. For an authoritative operational snapshot, verify provenance, immutable lineage, SHA-256, schema, freshness, PII/secret safety, and a bounded sample under `consume-operational-snapshots`, then continue to step 7. Do not force Athena or operational telemetry through a Lexicon ruleset or invent a mapping.
...
7. **Build and test optimized source queries before comparing results.**
   - For an authoritative operational snapshot, do not query the upstream API. `HeadObject` and `GetObject` the current and immutable S3 objects, verify declared and computed SHA-256, validate schema/updater version, exact window, freshness, summary invariants, and a bounded sample, and fail closed on any mismatch.
   ...
   - Run the smallest safe Athena or Persist smoke/profile query, or operational-snapshot schema/sample validation, needed to test the plan before launching broad comparisons: confirm the candidate count, sample row shape, and any report-specific fields, metrics, date semantics, statuses, joins, grouping keys, or definitions that require clarification. Do not run comparisons or target scenarios until the source plan has passed this bounded test.
...
8. **Confirm report intent and build the dataset contract.**
   ...
   - For each widget, write one dataset contract that names the source. For Athena include catalog/database/table, entity and ID semantics, partition predicates, selected run/filter lineage when applicable, metric calculation, grouping, sorting, row limit, freshness, and result provenance. For operational snapshots include current/immutable S3 URIs, source, schema/updater version, exact window, SHA-256 verification, field authority/heuristic status, metric calculation, grouping, sorting, row limit, and freshness. For other paths include the source ruleset/filter/separate rule or direct Lexicon label/index and its release/source details.
...
9. **Generate local JSON/CSV artifacts and build the local preview.**
   ...
   - For operational-snapshot-backed widgets, transform only the verified source facts needed by each dataset contract. Do not call the upstream external API or silently reinterpret source-owned fields.
...
- For an approved communication/call/payment or payment-plan snapshot count or date filter, reuse or establish a production profile through the normal Hoothoot AWS access flow, verify it in `us-east-2` against account `014948052063`, then inspect Athena/Glue metadata under the canonical skill. For an operational snapshot, verify private S3 access and the full snapshot contract under `consume-operational-snapshots`. For other report data, use prod Persist/Rules by default and offer the same supported credential choices.
...
- Load `skills/consume-operational-snapshots/SKILL.md` whenever the requested report concerns non-business operational/control-plane telemetry and a source owner has staged a private S3 snapshot.
- Keep source production separate from reporting: the updater owns PagerDuty, GitHub, CI, or another upstream API; Hoothoot owns S3 verification, report transformation, UI, and protected publishing.
- Never treat an arbitrary local JSON file, user upload, mutable current object without immutable lineage, or business-domain export as an authoritative operational snapshot.
- Do not force qualifying operational telemetry into Persist. Persist/Rules remains mandatory for business-domain data.
- Make freshness visible. For a fixed snapshot, state that the report will not change until the source owner publishes another snapshot and an on-demand report refresh runs.
...
- Load `skills/access-orchestrate-call-outputs/SKILL.md` whenever a request may be an approved communication/call/payment count, date filter, payment-plan lifecycle inventory, or payment-plan / installment snapshot question. Treat that skill as the source of truth for discovery commands, current schema facts, lineage rules, safety, and query examples; do not duplicate or improvise its runbook here.
...
- Prefer Athena for those entity counts, payment-plan lifecycle/snapshot questions, and easy date filters. Discover business identifiers and timestamps; count rows and distinct business identifiers separately when they differ.
...
- For Payment Plan Inventory / monthly additions / endings / amended-recovered / in-force-at-cutoff, query `orchestrate_call_outputs.payment_plan_lifecycle_events` (release `v20260722-lifecycle-1-business`; grain debt + plan + status + `effective_at`; **populated through 2026-07-21**; static). Do not use snapshot tables for those semantics.
- For current plan/installment status or amounts only, use `payment_plan` / `payment_plan_installment` (release `v20260721-initial-1-business`; also through **2026-07-21** Stage cutoff). Not lifecycle history and not future-obligation coverage.
...
Treat the first report iteration as a local preview backed by an approved live source, not a deployment. After collecting the local project path, Hoothoot may create the new report scaffold and query files immediately, but real business numbers must come only from tested Athena or Persist/Rules queries, and operational telemetry must come only from a verified authoritative snapshot. The first data implementation step MUST verify AWS credentials, discover live Athena metadata, verify the operational-snapshot contract, or resolve Lexicon rulesets/filters/separate rules as applicable before generating final JSON/CSV artifacts or comparing results.
...
- During preview, query Athena/Persist, read Rules outputs, or fetch a verified operational snapshot only enough to validate the report shape and numbers. Cache generated artifacts for design iteration, and rerun expensive queries, executions, or snapshot reads only when the field mapping, filters, aggregation logic, or source version changes.
...
- For operational-snapshot-backed widgets, name the current and immutable S3 URIs, source, schema/updater version, exact window, declared/computed SHA-256, freshness, field authority/heuristic status, and validation assumptions.
...
- Include example JSON/CSV shapes only for tests or schema documentation. Do not use example data as the user-facing report preview when the approved Athena, operational-snapshot, or Persist/Rules source is unavailable.
...
- Compact audit/details provenance per operational-snapshot widget: current/immutable S3 URIs, source, schema/updater version, exact window, declared/computed SHA-256, fetched-at time, AWS account/region/profile, freshness, and validation status.
...
- Athena, operational-snapshot, or Persist dataset contract; live metadata/snapshot verification/Lexicon discovery result; SQL, S3 lineage, or Gremlin query; polling plan; lineage policy; and assumptions.
- Timing summary split by Athena metadata/query work, operational-snapshot head/get/hash/schema validation, Lexicon resolution, Rules read/execution, each Persist query or direct filter/rule execution, artifact generation, deployment, static upload, and first deployed refresh.
