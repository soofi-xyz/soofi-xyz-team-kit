...
12. Add a local preview workflow as the default:
   ...
   - When the user wants real report data locally, own the prod credential check instead of handing them a command runbook:
     ...
     - If the user does not have an existing credential file and does not know the AWS access portal/start URL or role, stop with a clear request for those exact values from their administrator. Do not send them a terminal runbook as the primary answer.
