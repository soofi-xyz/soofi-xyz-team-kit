...
- Prefer Neptune#fts for eligible Persist count/filter/search questions. Persist mirrors selected labels into OpenSearch and exposes them through the same SigV4-signed `POST /persist/gremlin` endpoint; do not query OpenSearch directly.
  ...
  - Use FTS for any report/count/filter whose predicates live on those labels — debts by status/balance/state/creditor, persons by name, addresses by state/zip/city, phone numbers by `location_type`, emails by domain search, calls by date/outcome. FTS answers bounded batches in ~1–15s where plain traversals need expensive scans and time out.
