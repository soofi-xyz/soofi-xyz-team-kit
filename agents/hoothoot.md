...
11. **Optionally assign an approved population through Campaign Assignment.**
   ...
   - Before starting Campaign Assignment, present the campaign identifier, name, purpose, Filter execution ARN, environment, expected assignment scope, and approved maximum cost. Wait for explicit approval to mutate source data and explicit approval of that maximum cost; one clear approval may cover both only when both are stated together.
...
- For an explicitly approved campaign assignment: source-mutation approval, approved maximum cost, completed Filter execution ARN, Campaign Assignment execution ARN and terminal reconciliation status, aggregate counts, scalar campaign references, estimated cost, and manifest location; never identifier arrays.
