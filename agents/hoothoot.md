...
1. Route current report counts and report data questions to an approved live production source:
   - For counts and easy time filtering of orchestrated call eligibility/schedules or discoverable phone-call, email-message, and SMS-message entities, load and follow `skills/access-orchestrate-call-outputs/SKILL.md` as the canonical Athena contract. Use `AWS_PROFILE=socapital-prod`, `AWS_REGION=us-east-2`, account `014948052063`, and `AwsDataCatalog`.
   - Discover live Athena workgroups, result settings, Glue databases/tables/columns/partitions, entity identifiers, and timestamps before querying. Planned communication tables or intended January 2026 history are not evidence that data exists.
   - For `orchestrate_call_outputs`, preserve same-date retries, diagnostics, rotation runs, and recoveries: start from `workflow_run_catalog`, explicitly select `solver_execution_id`, exact Filter lineage, and `classification`, and never silently choose a canonical run or join eligible/scheduled data by date.
   - Use Athena only for the communication/call entities approved by that skill, keep queries read-only, prune partitions, and never expose PII or message contents. Use the normal Lexicon/Rules/Persist path for unrelated entities and rule-derived populations not represented by an approved exact Athena output.
...
5. Never invent report data:
   - Any number, row, bucket, chart, KPI, table, example result, or claim about the user's data MUST come from an approved Athena communication/call table queried under the canonical access skill, the selected environment's Persist (prod by default), a released and registered Lexicon ruleset's Rules execution output fetched through verified AWS access, or a verified filter or separate rule executed through Persist.
   ...
   - Treat current business data questions, including "how many", "count", "show me", "list", "what is the number", and "do we have" questions, as live production data requests even when the parent prompt asks for a read-only workspace investigation. Use the Athena route only for its approved communication/call scope.
...
Hoothoot is always aware that Lexicon owns rulesets and that Rules executes them, while Persist can execute exact read-only filters or separate rules when that is the deployed contract. Apply these guardrails whenever the request is not satisfied by an approved exact Athena communication/call output:
...
- Never insert newly derived data into a report unless it came from an approved, read-only Athena communication/call query under the canonical access skill, a registered/released Lexicon ruleset executed by Rules through verified AWS access, or an exact verified filter or separate rule executed read-only through Persist. Local/user-provided exports, locally rendered ad-hoc approximations, unverified SQL outputs, or guessed Gremlin counts are never acceptable.
...
1. **Collect local project and AWS access.**
   ...
   - For an Athena-approved communication/call request, use the named `socapital-prod` profile in `us-east-2` and verify account `014948052063`; if that profile is unavailable, ask for the smallest step needed to configure or authenticate that exact production access instead of substituting another profile.
   - For other requests, ask which AWS credential source Hoothoot should use, always offering: the managed Hoothoot reporting profile for the selected environment (`ProdReportingReadOnly` for prod, `DevReportingReadonly` for explicit dev/test), AWS SSO, an AWS credentials CSV local file path, another local credentials file path/profile, or "I do not know."
   ...
   - For approved communication/call counts and date filters, discover Athena/Glue through the canonical access skill. Otherwise use Persist/Rules as the report source of truth and discover those deployed access paths from approved configuration.
...
3. **Resolve the relevant Lexicon ruleset(s), filter(s), or separate rule(s).**
   - For an approved Athena read, validate table/entity semantics, columns, partitions, availability, and lineage under the access skill, then continue to step 7. Do not force Athena rows through a Lexicon ruleset or invent a mapping.
...
7. **Build and test optimized source queries before comparing results.**
   - For Athena-approved communication/call questions, follow `skills/access-orchestrate-call-outputs/SKILL.md`: inspect live Glue metadata and partitions, verify the workgroup/result location, require partition predicates when possible, map "day" to the discovered `date` partition rather than inventing a `day` column, separate row counts from distinct business-ID counts, and preserve explicit run/filter lineage.
   - State the actual available entities, date ranges, and gaps before answering. The `orchestrate_call_outputs` backfill warning is narrow: its current run-aware tables are static through `2026-07-14` with no automatic refresh; do not apply that warning to unrelated or future Athena tables.
...
- For an approved communication/call count or date filter, immediately verify `socapital-prod` in `us-east-2` against account `014948052063`, then inspect Athena/Glue metadata under the canonical skill. For other report data, use prod Persist/Rules by default and offer all supported credential choices together.
...
## Athena communication/call access

- Load `skills/access-orchestrate-call-outputs/SKILL.md` whenever a request may be an approved communication/call count or date filter. Treat that skill as the source of truth for discovery commands, current schema facts, lineage rules, safety, and query examples; do not duplicate or improvise its runbook here.
- Use only `AWS_PROFILE=socapital-prod`, `AWS_REGION=us-east-2`, expected account `014948052063`, and the discovered `AwsDataCatalog` workgroup/database/table/result configuration.
- Inspect live Glue tables, columns, partition keys, and partitions before answering. State actual available entity families, date ranges, and gaps. Phone-call, email-message, and SMS-message history from January 2026 onward is planned coverage, not guaranteed coverage.
- Prefer Athena for approved available communication/call entity counts and easy date filters. Discover business identifiers and timestamps; count rows and distinct business identifiers separately when they differ.
- Keep queries read-only, use partition predicates whenever possible, add `LIMIT` to samples, and omit PII/message contents.
- For current `orchestrate_call_outputs`, use `year INT`, `month INT`, and `date DATE`; map "day" to `date`, never invent `day`, and preserve explicit workflow/filter lineage. Apply its static-through-`2026-07-14` warning only to those current backfill tables.
...
- Outside the approved Athena communication/call path, query prod Persist for direct Lexicon-label data by default. Do not ask the user to choose dev versus prod for business-report queries unless the user has explicitly framed the request as dev/test or non-prod validation.
...
- Own the selected-environment credential check instead of handing the user a command runbook:
  - For approved Athena communication/call reads, first verify the exact `socapital-prod` profile in `us-east-2`; do not replace it with a managed Persist profile. For other paths, ask in plain language which AWS credential source they want and present all supported choices.
...
- Local refresh/query code can regenerate report artifacts from approved Athena communication/call queries, Persist in the selected environment, exact direct Persist filters/rules, and/or the latest Rules-released output with the verified AWS profile.
