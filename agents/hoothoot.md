...
7. **Build and test optimized Persist queries before comparing results.**
   - For every Persist-backed report question, first draft the optimized query plan from the validated Lexicon labels, properties, indexes, and edge paths. Prefer Neptune#fts for count/filter/search predicates on FTS-mirrored vertices (`person`, `phone_number`, `email`, `address`, `debt`, `payment`) and edges (`phone_call`, `payment_status_changed`, `text_message_status_changed`, `email_message_status_changed`); otherwise prefer indexed root filters, bounded projections, one focused dataset per widget, and sharded async queries for edge/sub-traversal workloads.
...
- Root-vertex-property-only report queries may aggregate directly when they are bounded and validated against Lexicon. If a query needs to traverse edges or run child/sub-traversals beyond the root vertices, do not run it as one whole-graph traversal. First run a root candidate count using only the root label and root-vertex filters, then split the root candidate stream into bounded `range(start, end)` shards such as `range(0, 50000)`, `range(50000, 100000)`, and `range(100000, totalCount)`. This `range(...)` sharding applies to plain non-FTS traversals only. Prefer Neptune#fts for large filtered counts/searches on FTS-mirrored labels, and batch broad FTS queries with disjoint indexed predicates instead of `range(...)`.
...
- Use Persist async Gremlin for reporting datasets by default:
  ...
  - Use `POST /persist/gremlin` only for small discovery or smoke-test queries that are expected to complete inside the synchronous timeout. Prefer sync for each bounded FTS batch that finishes in ~1–15s. Split broad FTS match sets before execution; use async only when one below-ceiling batch has additional graph work that needs more runtime.
...
- Prefer Neptune#fts for eligible Persist count/filter/search questions. Persist mirrors selected labels into OpenSearch and exposes them through the same SigV4-signed `POST /persist/gremlin` endpoint; do not query OpenSearch directly.
  - Indexed scope (~1.8B documents after the July 2026 payment/status backfill; freshness typically seconds; responses carry `fts.syncLagSeconds`): vertices `person`, `phone_number`, `email`, `address`, `debt` (configured Lexicon fields plus derived indexes), `payment` (`debt_identifier`, `payment_identifier`, `amount`, `payment_method`, `payment_provider`, `effective_at`); edges `phone_call` (all configured fields), `payment_status_changed` (`status`, `effective_at`, `revenue_delta`, `reason_code`, `created_at`), `text_message_status_changed` (`status`, `effective_at`, `created_at`), and `email_message_status_changed` (`status`, `effective_at`, `created_at`).
  - Use FTS for any report/count/filter whose predicates live on those labels — debts by status/balance/state/creditor, payments by method/provider/amount/date, payment status changes by status/revenue delta/reason/date, communication status changes by status/date, persons by name, addresses by state/zip/city, phone numbers by `location_type`, emails by domain search, and calls by date/outcome. FTS answers bounded batches in ~1–15s where plain traversals need expensive scans and time out.
  ...
  - Hard result ceiling: one FTS/OpenSearch query returns at most 10,000 matches, including a traversal ending in `.count()`. Treat exactly `10,000` as a truncated lower bound. Async execution does not remove this ceiling.
  - FTS batching contract: partition broad queries into disjoint indexed predicates (prefer half-open time windows), recursively split every batch that returns `10,000` until each is below the ceiling, then sum only non-overlapping counts or merge rows with business-key deduplication. Do not use `range(10000, ...)` or `Neptune#fts.maxResults` as pagination workarounds.
  - Pitfalls: `values('x').dedup()` after FTS breaks pushdown (times out; use `dedup().by('x')`); never supply `Neptune#fts.endpoint` (policy rejects); do not set `Neptune#fts.maxResults` above 1000 (policy cap, and it does not bypass the 10,000-match ceiling); `Neptune#fts.sortBy` needs the `.value` suffix for non-string fields.
