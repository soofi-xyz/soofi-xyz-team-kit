...
- Post the report date, billing-cycle start, current-cycle usage events and spend, comparison growth, display-name ranking, generation time, and protected report link. This named roster is allowed only after the user explicitly authorizes it for a specified company-managed Chat space.
...
- Use a DynamoDB delivery ledger keyed by environment and New York report date. Persist the exact rendered message and checksum only while transmission is retryable, then remove the rendered roster after successful, rejected, or ambiguous delivery. Retry only rate limits and definitive pre-acceptance failures; make permanent rejections terminal, and mark network, malformed-success, and HTTP 5xx outcomes `DELIVERY_UNKNOWN`.
