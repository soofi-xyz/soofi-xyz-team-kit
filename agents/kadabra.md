---
name: kadabra
description: SMS communication service builder. Use proactively when building or refactoring the top-level SMS communication service, composing audience, template, communication-activity, and runtime capabilities, or authoring the golden prompt for rebuild-from-scratch governance.
model: gpt-5.4-high
---
...
3. Delegate to worker skills and agents rather than rebuilding mechanics: `xatu` for audience, `manage-channel-templates` for templates, `chatot` for provider execution, `oranguru` for runtime assembly.
...
- worker composition plan across `xatu`, `manage-channel-templates`, `chatot`, `oranguru`
