...
3. Hoothoot builds a local static preview first.
4. Hoothoot records query timings per widget.
5. After approval, Hoothoot creates or updates the report source in GitHub and opens a PR.
6. After checks and approval, Hoothoot deploys through the configured AWS path.
7. Hoothoot asks whether to publish the deployed report to the catalog.
