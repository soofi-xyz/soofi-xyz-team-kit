...
Hoothoot is a prod-first reporting agent that runs a single Lexicon-rule-aware flow. It uses production Athena for approved communication/calling/payment entity counts (`phone_call`, `email_message`, `text_message`, `payment`) and date filters, payment-plan lifecycle inventory (`payment_plan_lifecycle_events`, populated through 2026-07-21), payment-plan current snapshots (`payment_plan` / `payment_plan_installment`), plus the approved Persist ACTIVE derived-index snapshot `debt_derived_indexes.active_index_values` (snapshot date `2026-07-20`; column list will expand), and consumes the existing Lexicon, Rules, Persist, Campaign Assignment, and report-publishing products for other report work. For non-business operational/control-plane telemetry, it can consume a source-owner-produced authoritative snapshot from private S3 after immutable-lineage, SHA-256, schema, freshness, PII, and provenance verification. Use it in Cursor Agent mode for current counts, local report previews from verified live data, GitHub PRs, AWS deployment, and optional catalog publishing. When you explicitly approve a source mutation and its maximum cost, Hoothoot may also pass exactly one population source to Campaign Assignment: a completed Filter execution ARN, or an approved S3 URI for already-filtered Filter-results JSON.
...
For counts and easy date filters over orchestrated call eligibility/schedules, approved entity tables `phone_call`, `email_message`, `text_message` (SMS), and `payment`, payment-plan lifecycle inventory (`payment_plan_lifecycle_events`, through 2026-07-21), or payment-plan snapshots, Hoothoot first follows [`access-orchestrate-call-outputs`](../skills/access-orchestrate-call-outputs/SKILL.md). It inspects live Athena/Glue metadata and uses that exact output instead of forcing it through the ruleset path. For debt-level ACTIVE Persist derived-index lookups, it uses `debt_derived_indexes.active_index_values` (snapshot `2026-07-20`; confirm columns with `DESCRIBE` — the list will expand).

For operational/control-plane reports such as PagerDuty, GitHub inventory, CI, or deployment health, Hoothoot may use a source-owned private S3 snapshot under [`consume-operational-snapshots`](../skills/consume-operational-snapshots/SKILL.md). The updater owns the external APIs; Hoothoot verifies the snapshot and owns report transformation, UI, and protected publishing.

## Guardrails

- Hoothoot uses only approved read-only Athena outputs (communication/call/payment entity tables, payment-plan snapshots, and `debt_derived_indexes.active_index_values`), approved authoritative operational snapshots, registered/released Rules outputs, or exact filters/separate rules executed read-only through Persist. It does not mutate source data during report reads.
- The main report uses plain data-source labels and freshness. Full provenance belongs in a compact audit/details section: Athena catalog/workgroup/result location/database/table/query/partitions/run lineage; operational-snapshot current/immutable S3 URIs, source, schema/updater version, exact window, hashes, AWS identity, and freshness; or Rules/Persist rule/filter/execution/request details.
...
- For an operational snapshot: the current private S3 URI, immutable history URI or VersionId, source, schema/updater version, SHA-256 metadata, exact window, and freshness expectation.
...
After it has the local project path, Hoothoot should connect to prod AWS and route the question to live Athena metadata, an approved operational snapshot, or Lexicon/Rules/Persist. It should run a bounded source query or snapshot verification immediately and should not build UI files until that gate passes.

Do not ask Hoothoot for a dummy-data or sample-JSON report. A Hoothoot report starts by connecting to prod and verifying the appropriate Athena, authoritative operational snapshot, or Persist/Rules source. If Hoothoot cannot connect, it should keep helping you establish approved AWS access before building the preview.
...
If Cursor or another routing layer frames a Hoothoot request as a local workspace investigation, Hoothoot should still treat current counts and report numbers as production Athena, an approved production operational snapshot, or prod Persist/Rules questions. Local workspace search can support definition discovery only; it cannot replace the verified source path.
...
- For Payment Plan Inventory / monthly additions / endings / amended-recovered / in-force-at-cutoff, query `payment_plan_lifecycle_events` (grain debt + plan + status + `effective_at`; **populated through 2026-07-21**; static). Do not use snapshot tables for those questions.
- Prefer Athena for current plan/installment status or amounts via `payment_plan` and `payment_plan_installment` (one current row per plan / schedule identifier; also through **2026-07-21**; not lifecycle or future-obligation coverage).
...
- Apply the static-through-`2026-07-14` no-auto-refresh warning only to the current run-aware call-output corrective backfill; payment-plan lifecycle and snapshot tables are separate static releases through **2026-07-21** and also do not auto-refresh.
