...
9. Use a reliable asynchronous Event → Queue persistence path. Emit a recoverable, versioned outcome-manifest event after producing the result artifact; make the queue consumer idempotently persist outcome records, retry transient failures, route exhausted work to a DLQ, and support reconciliation/replay. Do not use fire-and-forget delivery or synchronous per-record writes in the Filter evaluation path.
