...
6. Apply the **Feature-branch testing contract** below on every feature PR.
   Place tests in the correct layer so BrowserStack is not overloaded. When
   pipeline layout drifts, run the Hoopa pipeline fixer (`hoopa audit` /
   `hoopa fix`) before claiming verification complete.

# Feature-branch testing contract — CRITICAL

Every feature branch must prove code correctness, design accuracy, API
integration, UI behavior, and mobile compatibility before merge. Required tests
must not silently skip. No test may use production data or submit a real
payment.

## 1. Unit tests

- Cover API handlers, services, validation, error mapping, and frontend
  JavaScript/TypeScript business logic.
- Cover success, validation, authorization, timeout, and error paths.
- Make **no network calls**.
- Changed-code coverage ≥ **90%**, with no reduction in repository coverage.
- Auth and payment business branches require complete success and error-path
  coverage.

## 2. Design and responsive tests

- Test every affected page against its approved Figma design or written
  specification (delegate execution patterns to `smeargle` /
  `skills/responsive-design-tests/`).
- Cover desktop, tablet, and mobile breakpoints.
- Check copy, spacing, colors, typography, visibility, scrolling, and control
  states.
- Visual comparisons use deterministic fixtures and controlled rendering
  tolerances. Any visual difference requires an approved baseline update.
- These tests remain **mocked** and do **not** claim API integration coverage.
- Do not run design suites in the BrowserStack PR gate.

## 3. Feature-preview integration tests

After Amplify (and any required isolated API stack) deploys the **exact**
feature commit:

- Run against the matching landing and portal preview URLs.
- Qualifying runs use **normal browser security** only — no disabled CORS,
  request interception, or CORS bridge. A separately labeled CORS-disabled
  diagnostic lane from `skills/unified-portal-smoke-testing/` may still run for
  diagnosis; it never replaces the normal-security qualifying result and never
  alone satisfies merge.
- List every frontend-consumed API in an endpoint manifest. Check each for
  authentication, CORS, status, schema, and frontend consumption.
- Use approved, stable DEV fixtures. Mutating operations use safe DEV data and
  never submit a real payment.
- Inventory and exercise every page, interactive control, navigation path, and
  link. Internal links verify the expected route; external links verify the
  expected destination and successful response.
- Required tests cannot silently skip.

## 4. BrowserStack real-device gate

Only these three critical Playwright journeys run continuously on BrowserStack
for every PR:

1. **Landing full flow** — landing login → portal handoff → authentication →
   account/home state.
2. **Portal full flow** — direct portal login → disclosures → home/navigation →
   payment setup boundary.
3. **Debt-ID login flow** — debt-scoped URL → identity verification → preserved
   debt context → disclosure skip.

Requirements:

- Run on the required Android Chrome and iPhone Safari devices.
- Use normal browser security and real DEV APIs.
- Tests are independent and deterministic.
- Helpers wait for valid states instead of using fixed sleeps.
- Broader device and scenario matrices run **nightly**, not in every PR gate.
- Design, full UI inventory, and extra smokes must not be added to the PR
  BrowserStack job.

## 5. Feature-branch test report

CI publishes one report on the PR containing:

- Exact commit and Amplify preview URLs (landing + portal)
- Unit-test results and coverage by application/API
- Changed-line coverage
- Design routes, breakpoints, and visual-diff results
- API endpoint contract matrix
- UI controls and links tested
- Integration scenarios and results
- BrowserStack devices, sessions, retries, and evidence links
- Required skips, failures, or quarantined tests
- Final merge verdict (`PASS` / `FAIL`)

## Definition of done (testing)

- [ ] All required layers pass
- [ ] No required test is skipped
- [ ] BrowserStack PR gate contains only the three defined journeys
- [ ] No test uses production data or submits a payment
- [ ] The complete test-and-coverage report is attached to the feature PR
...
8. **Feature verification.** Place and run tests per the Feature-branch testing
   contract: unit (no network), design (mocked; smeargle), feature-preview
   integration (normal browser security against exact Amplify/API preview), and
   BrowserStack PR gate (**only** landing-full-flow, portal-full-flow,
   debt-id-login-flow on required Android Chrome + iPhone Safari). Create and
   commit executable integration tests from every supplied story scenario. Run
   them against the exact feature deployment with a normal-security baseline as
   the qualifying lane. An optional CORS-disabled Chrome diagnostic from
   `skills/unified-portal-smoke-testing/` may be recorded separately; a
   CORS-disabled pass does not prove CORS correctness and does not replace the
   normal-security result. Also run repository gates plus scope-appropriate
   latency and IaC checks. Broader BrowserStack matrices are nightly only.
   Publish the single feature-branch test report on the PR. Live 401 expected /
   404 received plus `{"message":"Not Found"}` means the GET route is missing
   at API Gateway. Unrelated landing BrowserStack React `#418`/`#423`/`#425` is
   not a feature regression.
9. **Pull request and handoff.** Push the feature branch, open or update the PR,
   and publish per-scenario feature evidence plus the feature-branch test
   report. Treat approval and development verification as required handoff
   phases: stop until explicit approval to proceed, and do not infer merge
   permission from test approval. After the feature commit is present on the
   development branch, rerun the same scenario IDs against the exact development
   deployment with normal browser security. Return attachment-ready evidence for
   every scenario in both environments and attach or link it to the Asana user
   story when authorized. Never merge without explicit approval.
...
2. If they do not exist, write named placeholders (`SECRET_PLACEHOLDER_ `) in CDK/env examples and a secrets checklist in the PR.
...
| Gate | Bar |
| --- | --- |
| Unit tests | Handlers/services/validation/error mapping/frontend business logic; success, validation, authz, timeout, and error paths; **no network**; changed-code coverage ≥ **90%** with no repository coverage regression; auth and payment branches fully covered |
| API unit tests | 100% pass when backend changes |
| Backend coverage | New portal ≥ 80% overall modules; existing project preserves its threshold with no regression and new backend modules ≥ 80%; never below the 90% changed-line floor above |
| Design tests | Mobile, tablet, desktop when frontend appearance changes; mocked fixtures; approved baseline updates for visual diffs; not claimed as API integration |
| Feature-preview integration | Exact feature Amplify (+ isolated API) previews; normal browser security; endpoint manifest + UI/link inventory; DEV fixtures only; no silent skips; no real payments |
| BrowserStack PR gate | **Only** landing-full-flow, portal-full-flow, debt-id-login-flow on required Android Chrome + iPhone Safari; normal security; real DEV APIs; state waits (no fixed sleeps); broader matrices nightly only |
...
| Feature-branch test report | One PR report with commit, preview URLs, coverage, design diffs, API matrix, UI inventory, integration results, BrowserStack evidence, skips/failures, and final merge verdict |
...
- [ ] Unit tests cover success/validation/authz/timeout/error paths with no network; changed-line coverage ≥ 90% and no repository coverage regression; auth/payment branches fully covered
- [ ] Design tests cover affected routes at desktop/tablet/mobile with mocked fixtures; visual diffs have approved baselines; design is not claimed as API integration
- [ ] Feature-preview integration ran against exact feature preview URLs with normal browser security (no CORS disable/bridge as the qualifying lane), endpoint + UI/link inventory, DEV fixtures only, and no silent skips or real payments
- [ ] BrowserStack PR gate ran only landing-full-flow, portal-full-flow, and debt-id-login-flow on required Android Chrome + iPhone Safari; broader matrices left to nightly
- [ ] Feature-branch test report is attached to the PR with commit, preview URLs, coverage, design, API matrix, UI inventory, integration, BrowserStack evidence, skips/failures, and final merge verdict
...
- [ ] Every scenario has feature evidence from the exact feature deployment; normal-security is the qualifying lane; any CORS-disabled diagnostic is labeled separately and not represented as CORS proof
