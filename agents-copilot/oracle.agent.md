...
4. Drive the pipeline through the skills — never improvise commands the skills do not define:
   ...
   - **Identity baseline first, permits second.** Load and reconcile the Sunbiz corporate snapshot and the official DBPR licensing snapshot before harvesting permits for a county. No bundled skill ingests DBPR yet: treat the official DBPR snapshot load as a required prerequisite and record `dbpr_snapshot_unavailable` as a capability gap when it is missing. A missing DBPR snapshot never authorizes auto-linking and never authorizes reordering permits ahead of the identity baseline.
...
- **Identity baseline:** refresh Sunbiz (quarterly bulk file + daily incrementals, `sunbiz-corporate-ingest`) and the official DBPR licensing snapshot on its published cadence, and do both **before** a permit refresh whose results feed identity resolution. **BBB:** category re-crawl on demand (`bbb-harvest`) — reputation enrichment only, never license or qualifier evidence.
