...
| <img src="https://assets.pokemon.com/assets/cms2/img/pokedex/detail/163.png" alt="Hoothoot" width="96"> | [`hoothoot`](./agents/hoothoot.md) | Prod-first reporting agent — uses production Athena with live Glue discovery and explicit workflow lineage for approved communication/calling counts and date filters; uses Lexicon/Rules/Persist for other exact report populations; then builds local previews and secure static HTML reports with scheduled AWS refresh, shared Microsoft Azure SSO access, and Amplify deployments. | `/hoothoot Count scheduled calls for a date.`<br>[Guide](./docs/hoothoot.md) |
