...
| <img src="https://assets.pokemon.com/assets/cms2/img/pokedex/detail/199.png" alt="Slowking" width="96"> | [`slowking`](./agents/slowking.md) | Candidate test-task evaluation orchestrator — extracts the task's core intent, enforces mandatory gates (PR to the task repo, a usable URL/path, working credentials or public access), drives the live product with Playwright to verify intent and data richness, dispatches a subagent to score whether the candidate used this kit correctly, and returns a weighted score with a hiring verdict. | `/slowking Evaluate this candidate test task...` |
...
| [`build-persist-service`](./skills/build-persist-service/) | Build the Persist graph-persistence platform service — Amazon Neptune backend with SigV4-authorised `/persist/*` HTTP API, lexicon-validated GraphSON v3 ingest (sync + async), content-addressed Persist Blobs, Neptune CSV bulk-load workflow, and sync + async Gremlin query channels. |
...
| [`evaluate-candidate-intent`](./skills/evaluate-candidate-intent/) | First phase of candidate test-task evaluation — extract the single core intent (the product that must deliver real value) and build a weighted acceptance checklist with mandatory gates: a PR to the task repo, a usable URL/path, and working credentials or public access. |
| [`evaluate-candidate-kit-usage`](./skills/evaluate-candidate-kit-usage/) | Kit-conformance phase (subagent) — classify whether the candidate built the task with this soofi-xyz kit by determining the expected agents/skills via arceus + README, consulting each relevant builder agent for a coding score, and aggregating a kit-usage conformance score. |
| [`evaluate-candidate-product`](./skills/evaluate-candidate-product/) | Functional-testing phase (subagent) — enforce the mandatory gates (no PR / no usable path / inaccessible = immediate 0), drive the live product with the Playwright browser to verify the core intent and acceptance criteria, and score data richness (toy datasets score extremely low). |
