...
**Elephant routing:** `donphan` + `use-elephant-mcp` = explore open data via MCP; `oracle` + `use-oracle` = ingest/refresh sources; `watchog` + `build-elephant-hero-facts` = build the scheduled homepage hero-facts service (monitor → verify → Asana approval → content-only PR); `use-elephant-query-db` = SQL over Neon.
...
| <img src="https://assets.pokemon.com/assets/cms2/img/pokedex/detail/163.png" alt="Hoothoot" width="96"> | [`hoothoot`](./agents/hoothoot.md) | Prod-first reporting agent — uses production Athena with live Glue discovery and explicit workflow lineage for approved communication/calling/payment counts (`phone_call`, `email_message`, `text_message`, `payment`), payment-plan lifecycle inventory (`payment_plan_lifecycle_events`, through 2026-07-21), payment-plan snapshots (`payment_plan` / `payment_plan_installment`), and date filters; uses Lexicon/Rules/Persist for other exact report populations; then builds local previews and secure static HTML reports with scheduled AWS refresh, shared Microsoft Azure SSO access, and Amplify deployments. After separate source-mutation and cost approval, it can pass exactly one source—a completed Filter execution ARN or an approved S3 URI for already-filtered Filter-results JSON—to Campaign Assignment without calling Persist ingest or launching a channel. | `/hoothoot Count scheduled calls for a date.`<br>[Guide](./docs/hoothoot.md) |
...
| 🔮 | [`oracle`](./agents/oracle.md) | Public-data ingestion agent — discovers, ingests, validates, and refreshes county property/permit records plus Florida Sunbiz and BBB contractor reputation into the Elephant Neon query DB by driving the `elephant-xyz/skills` (`onboard-county` + stage skills) on the `oracle-node` pipeline. Lee County, FL is the reference. IPFS/indexing/MCP/NEO are out of scope. | `/oracle Onboard Lee County, FL...` |
...
| [`manage-story-quality`](./skills/manage-story-quality/) | Operating guide for the WOW Story Quality Agent in `wow-website` — LLM rewrite of new Asana tasks into the WOW story format with a creator-assigned review subtask; covers org/project onboarding, webhook + sweeper triggers, the idempotent ledger, diagnosis, and format changes. |
| [`operate-livevox-campaigns`](./skills/operate-livevox-campaigns/) | Operate SOC LiveVox campaigns safely: inventory MDC and Solver states/stats, start or stop exact campaigns, refill remaining hours from graph-filtered data, deduplicate debt IDs, continue current-hour tiers, and verify Integrate uploads. |
