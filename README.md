...
**Elephant routing:** `donphan` + `use-elephant-mcp` = explore open data via MCP; `oracle` + `use-oracle` = mine county sources into lexicon records, validate them, publish one archive per county run plus its CLI-exported per-class tables, and run the existing query-table/coverage publication; `build-county-transform` = author and prove county transforms; `watchog` + `build-elephant-hero-facts` = build the scheduled homepage hero-facts service (monitor → verify → Asana approval → content-only PR); `use-elephant-query-db` = SQL over Neon.
