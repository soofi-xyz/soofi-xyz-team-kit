...
**Elephant routing:** `donphan` + `use-elephant-mcp` = explore open data via MCP; `oracle` + `use-oracle` = mine county sources into lexicon records, validate them, upload one archive per county per data group plus its CLI-exported per-class tables, register every group on the county's Atlas page by pull request, and run the existing query-table/coverage publication; `build-county-transform` = author and prove county transforms; `watchog` + `build-elephant-hero-facts` = build the scheduled homepage hero-facts service (monitor → verify → Asana approval → content-only PR); `use-elephant-query-db` = SQL over Neon.
...
**Data product routing:** `lapras` + `build-connect-product` owns Connect ingestion
(registered sources → JDBC/PySpark deltas and entity bundles → downstream-gated
checkpoints, samples and maintained snapshots). The separate partner API/webhook/
SFTP Connect service remains with `conkeldurr` + `build-connect-service`.
...
| <img src="https://assets.pokemon.com/assets/cms2/img/pokedex/detail/131.png" alt="Lapras" width="96"> | [`lapras`](./agents/lapras.md) | Connect ingestion specialist — Stage-derived PySpark extraction through registered adapters (PostgreSQL JDBC, S3 Parquet/CSV/Excel), registered source/entity contracts, record deltas, dependency hydration, downstream-gated checkpoints, samples and Iceberg snapshots. | `/lapras Build Connect ingestion from registered JDBC or S3 file sources through verified checkpoint delivery...` |
...
| <img src="https://assets.pokemon.com/assets/cms2/img/pokedex/detail/151.png" alt="Mew" width="96"> | [`mew`](./agents/mew.md) | Read-only universal lexicon architect — turns a plain-language use case from any industry into a reusable core, composable domain extensions, explicit identities and lifecycles, and auditable chain-of-custody events. | `/mew Model a healthcare appointment and communication extension...` |
...
| 🔮 | [`oracle`](./agents/oracle.md) | Public-data mining agent — turns a county's property, permit, corporate-registry, and contractor sources into validated lexicon records by driving the bundled stage skills under `skills/` against `skills/use-oracle/runtime/`, packs one archive per county per data group with its per-class tables, and registers every group on the county's Atlas page by pull request. Keeps the existing query-table and coverage publication into the `elephant` MCP running unchanged. Fail-closed readiness is required before seed or ingest; identity baseline precedes permits. Lee County, FL is the reference. Replacing the existing publication path and on-chain indexing are out of scope. | `/oracle Onboard Lee County, FL...` |
...
| [`build-connect-product`](./skills/build-connect-product/) | Build Connect ingestion in a target repository from its Stage-derived specification — exact source/run/checkpoint contracts, PySpark extraction/hydration, tracked observations, samples, snapshots, build sequence and acceptance examples. |
