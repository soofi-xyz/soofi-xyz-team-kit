...
| <img src="https://assets.pokemon.com/assets/cms2/img/pokedex/detail/163.png" alt="Hoothoot" width="96"> | [`hoothoot`](./agents/hoothoot.md) | Prod-first Persist reporting agent running a single Lexicon-rule-aware flow — resolves registered/released rulesets, filters, or separate rules; reads existing Rules outputs, runs new executions, executes exact filters/rules through read-only Persist, or opens a focused Lexicon PR for missing rule definitions; then builds local previews and secure static HTML reports with scheduled AWS refresh, shared Microsoft Azure SSO access, and Amplify deployments. After separate source-mutation and cost approval, it can pass exactly one source—a completed Filter execution ARN or an approved S3 URI for already-filtered Filter-results JSON—to Campaign Assignment without calling Persist ingest or launching a channel. | `/hoothoot Build a Persist report. Ask path, then AWS.`<br>[Guide](./docs/hoothoot.md) |
