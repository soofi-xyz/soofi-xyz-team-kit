...
| [`county-readiness-preflight`](./skills/county-readiness-preflight/) | Fail-closed validator for `elephant-pipeline/docs/<county>-sources.yaml`. `onboard-county`, `county-seed-data`, and `county-ingest-run` must run it before seed, pilot, or full ingest. |
| [`use-oracle`](./skills/use-oracle/) | Operating guide for the Oracle ingestion agent: choose one stack, install and drive `elephant-xyz/skills` (`onboard-county` + stage skills), enforce YAML readiness, ingest/refresh county property/permit plus Sunbiz/BBB data, then publish after human approval plus automated upload. On-chain indexing and NEO out of scope. |
