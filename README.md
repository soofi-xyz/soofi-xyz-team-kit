...
| [`candidate-agent-qc-validation`](./skills/candidate-agent-qc-validation/) | Evaluate hiring-candidate-built agents against requirements with timed submission scoring (GitHub last-commit), per-criterion results, structure/runtime/network dimension scores, scalability discounts, weighted overall performance %, compensation recommendation (capped at $250k), and a concise hiring signal. |
