...
**Donphan / Elephant MCP:** Requires Node **22.18+**. Bundled `mcp.json` runs Elephant MCP 2.0 from GitHub `main`, resolves the global Atlas IPNS, and synchronizes verified normalized tables into a home-directory SQLite database. After install or `git pull`, reload Cursor and confirm **`elephant`** is enabled under **Settings → MCP**. Discover county/data-group scope with `listPublishedCounties`, then pass `county`, `dataGroup`, and table scope explicitly.
...
| [`deploy-open-data-mcp`](./skills/deploy-open-data-mcp/) | Run or deploy Elephant MCP 2.0 with global Atlas IPNS synchronization into SQLite or Postgres/Neon. |
...
| [`use-elephant-mcp`](./skills/use-elephant-mcp/) | Explore synchronized Atlas data through normalized Elephant MCP 2.0 tools. |
