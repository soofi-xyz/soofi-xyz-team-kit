...
| 🔮 | [`oracle`](./agents/oracle.md) | Public-data ingestion agent — discovers, ingests, validates, and refreshes county property/permit records plus Florida Sunbiz and BBB contractor reputation into the Elephant Neon query DB by driving the `elephant-xyz/skills` (`onboard-county` + stage skills) on the `oracle-node` pipeline, then publishes the county query table and coverage snapshot and wires them into the `elephant` MCP. Lee County, FL is the reference. On-chain indexing beyond the query table and NEO are out of scope. | `/oracle Onboard Lee County, FL...` |
...
| [`use-oracle`](./skills/use-oracle/) | Operating guide for the Oracle ingestion agent: install and drive the `elephant-xyz/skills` (`onboard-county` + stage skills) on the `oracle-node` pipeline to discover, ingest, validate, and refresh county property/permit plus Sunbiz/BBB data into the Neon query DB, then publish the query table and coverage snapshot. On-chain indexing and NEO out of scope. |
