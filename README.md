...
| <img src="https://assets.pokemon.com/assets/cms2/img/pokedex/detail/163.png" alt="Hoothoot" width="96"> | [`hoothoot`](./agents/hoothoot.md) | Prod-first reporting agent — uses production Athena with live Glue discovery and explicit workflow lineage for approved communication/calling/payment counts (`phone_call`, `email_message`, `text_message`, `payment`), payment-plan lifecycle inventory (`payment_plan_lifecycle_events`, through 2026-07-21), payment-plan snapshots (`payment_plan` / `payment_plan_installment`), and date filters; consumes integrity-verified source-owner S3 snapshots for non-business operational telemetry; and uses Lexicon/Rules/Persist for other exact report populations. It builds local previews and secure static HTML reports with on-demand or scheduled AWS refresh, shared Microsoft Azure SSO access, and Amplify deployments. After separate source-mutation and cost approval, it can pass exactly one source—a completed Filter execution ARN or an approved S3 URI for already-filtered Filter-results JSON—to Campaign Assignment without calling Persist ingest or launching a channel. | `/hoothoot Build an operational report from a verified S3 snapshot.`<br>[Guide](./docs/hoothoot.md) |
