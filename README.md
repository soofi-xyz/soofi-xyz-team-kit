...
| <img src="https://assets.pokemon.com/assets/cms2/img/pokedex/detail/163.png" alt="Hoothoot" width="96"> | [`hoothoot`](./agents/hoothoot.md) | Prod Persist-only reporting agent running a single Lexicon-rule-aware flow — resolves registered/released rulesets, reads existing Rules outputs or runs new executions, opens a Lexicon PR only when the user explicitly adds/changes a rule, then builds local previews and secure static HTML reports with scheduled AWS refresh, shared Microsoft Azure SSO access, and Amplify deployments. | `/hoothoot Build a Persist report. Ask path, then AWS.`<br>[Guide](./docs/hoothoot.md) |
