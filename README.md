...
| <img src="https://assets.pokemon.com/assets/cms2/img/pokedex/detail/163.png" alt="Hoothoot" width="96"> | [`hoothoot`](./agents/hoothoot.md) | Prod-first reporting agent — uses production Athena with live Glue discovery and explicit workflow lineage for approved communication/calling counts and date filters; uses Lexicon/Rules/Persist for other exact report populations; then builds local previews and secure static HTML reports with scheduled AWS refresh, shared Microsoft Azure SSO access, and Amplify deployments. After separate source-mutation and cost approval, it can pass exactly one source—a completed Filter execution ARN or an approved S3 URI for already-filtered Filter-results JSON—to Campaign Assignment without calling Persist ingest or launching a channel. | `/hoothoot Count scheduled calls for a date.`<br>[Guide](./docs/hoothoot.md) |
...
| Skill | Description |
| --- | --- |
| [`access-orchestrate-call-outputs`](./skills/access-orchestrate-call-outputs/) | Query approved production Athena communication/calling entities for counts and date filters with live Glue discovery, partition pruning, explicit workflow lineage, read-only safety, and compact CLI/SQL examples. |
