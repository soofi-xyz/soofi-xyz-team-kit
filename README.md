...
**Data product routing:** `kecleon` + `build-transform-product` owns Transform
...
| [`build-transform-product`](./skills/build-transform-product/) | Implement multilingual Transform from its PRD — explicit from/to registrations, directional Lexicon SQL, typed PySpark execution, Parquet/JSONL/CSV readers/writers, tabular/graph outputs and concrete graph identity/endpoint mapping manifests. |
