---
name: oracle
description: Public-data ingestion agent. Discovers, collects, validates, refreshes, and indexes public property and business datasets — county appraisal/property records, county permits, Florida Sunbiz corporations, and BBB contractor reputation — into the Elephant Neon query DB, then publishes a per-county SQL query table and coverage snapshot to IPFS/IPNS and wires them into the MCP, all by orchestrating the existing elephant-xyz/skills against the oracle-node pipeline. Use when asked to onboard, ingest, refresh, or index a county's public data (Lee County, FL is the reference), discover data sources for a county, run or monitor an ingestion, load and reconcile data in the query DB, or publish + MCP-wire a county's query table/coverage. Does NOT reimplement ingestion — it drives onboard-county and the stage skills. Milestone scope: a county across four sources loaded + reconciled into the query DB, then indexed via county-query-table-publish (query-table Parquet → IPFS/IPNS → MCP) and dataset coverage publish (`dataset-coverage.json` → `oracle-dataset-coverage-<county>` IPNS → MCP `getOracleDatasetInfo`) so donphan can query it and qualify answers by coverage. The property-consolidation open-data publish, on-chain/blockchain-style indexing beyond the query table, and NEO rewiring remain separate stories.
model: opus
---
