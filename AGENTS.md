...
- Each source agent MUST have a matching Codex specialist skill at `plugins/soofi-xyz-team-kit/skills/agent-<name>/SKILL.md`.
- After adding, removing, renaming, or editing agents, run `scripts/sync-copilot-agents.sh sync`, `scripts/sync-codex-agents.sh sync`, and `python3 scripts/sync-codex-skills.py sync` to refresh generated targets.
...
- Codex repo marketplace points at `plugins/soofi-xyz-team-kit/`, a materialized package with real files. Do not replace its manifest or skill files with symlinks.
- Codex custom agents are project-scoped TOML files in `.codex/agents/` generated from `agents/`.
- Codex specialist skills are generated from `agents/` into the Codex package. Shared skills and supporting files are copied there from the repository root. Edit canonical sources, then run `python3 scripts/sync-codex-skills.py sync`; do not edit the package directly.
...
├── plugins/
│   └── soofi-xyz-team-kit/           # Materialized Codex-only plugin package
...
Verify the installed plugin contains the `agent-<name>` skills, then start a new Codex thread so it reloads them. Project-scoped custom agents remain available when working inside this repository.
...
- [ ] `plugins/soofi-xyz-team-kit/skills/agent-<name>/SKILL.md` exists for every source agent, generated by `python3 scripts/sync-codex-skills.py sync`.
...
- [ ] Plugin installs locally via the Codex repo marketplace and its installed cache contains the expected skills.
