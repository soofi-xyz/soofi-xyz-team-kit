...
- Each source agent MUST have a matching Codex specialist skill at `skills/agent-<name>/SKILL.md`.
...
- Codex repo marketplace points at the repository root so its installed package contains real skill files. Do not point it at the symlinked `plugins/soofi-xyz-team-kit/` compatibility folder.
- Codex custom agents are project-scoped TOML files in `.codex/agents/` generated from `agents/`.
- Codex specialist skills are generated from `agents/` into `skills/agent-<name>/`; edit the source agent, not the generated skill.
...
├── plugins/
│   └── soofi-xyz-team-kit/           # Legacy symlinked compatibility folder; marketplace uses repo root
...
- [ ] `skills/agent-<name>/SKILL.md` exists for every source agent, generated by `python3 scripts/sync-codex-skills.py sync`.
