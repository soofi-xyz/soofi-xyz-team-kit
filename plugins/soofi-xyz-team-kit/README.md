...
The Codex marketplace installs a self-contained package from `plugins/soofi-xyz-team-kit/`. Its real `skills/` directory contains copies of the shared skills and generated `agent-<name>` specialist skills for all source agents. Invoke a specialist as `$soofi-xyz-team-kit:agent-<name>` from any project after starting a new Codex thread. The generated specialist skills stay out of the root `skills/` directory used by Cursor and Copilot CLI. The source agents remain native subagents in those harnesses; `.codex/agents/` also contains project-scoped Codex custom agents for work inside this repository.
