...
oracle_paths_main() {
  ...
  if [[ ! -f "${plugin_root}/.cursor-plugin/plugin.json" && ! -f "${plugin_root}/.codex-plugin/plugin.json" ]]; then
    echo "oracle-paths.sh: resolved SOOFI_PLUGIN_ROOT (${plugin_root}) is missing a Cursor or Codex plugin manifest; refusing to export unverified paths" >&2
    return 1
  fi
