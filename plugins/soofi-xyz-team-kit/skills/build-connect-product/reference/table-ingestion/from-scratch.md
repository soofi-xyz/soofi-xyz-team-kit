...
| 4. `glue_scripts/connect_worker.py`, `glue_scripts/readers/{postgres_jdbc,s3_file}.py` | Shared local/Glue entry point; one reader module per adapter behind the five-method protocol, normalization, schema/scope checks and materialization. The `s3_file` reader is also the local fixture reader | Typed full baseline through both adapters with no credentials in local mode |
