...
| Spark is not installed globally even though the pinned Transform runtime supplies a bounded setup script | Run the declared local setup in the isolated checkout and verify its Spark version before executing fixtures | Continue locally; `BLOCKED` only when setup is absent, incompatible, or fails |
