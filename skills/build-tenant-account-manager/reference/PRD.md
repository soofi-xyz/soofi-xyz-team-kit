...
2. **API key management** — manage the full lifecycle of every API key: account-bound keys (the public `x-api-key` value any caller uses to identify the originating account) and per-account **service keys** (used by Staircase products inside one account to call other products in the same account or across accounts). Keys are minted by Account, bound to the right API Gateway Usage Plan via custom-resource Lambdas, encrypted at rest, and rotatable via a dedicated workflow that never exposes plaintext to storage.
...
4. **DNS configuration** — for each provisioned account, create a Route 53 hosted zone for `<subdomain>.staircaseapi.com`, request the matching ACM main + wildcard certificates, perform DNS-validation, and publish the account's canonical domain inventory for downstream product deployments. Individual service CDK stacks own their own API Gateway `AWS::ApiGatewayV2::ApiMapping` resources.
5. **Maintenance access** — mint short-lived, MFA-gated AWS console credentials into any provisioned account on demand for support / on-call engineers. The mint runs **from the marketplace** by `sts:AssumeRole`-ing into the tenant sub-account's `OrganizationAccountAccessRole`, then provisioning a temporary IAM user, login profile, and virtual MFA device. Each session is owner-approved (via SES email callback), single-use, and TTL-bounded; subsequent requests by the same `username` against the same account supersede the prior session.
...
| **Service keys**            | `GET /accounts/{account_id}/service-key`, `POST /accounts/{account_id}/service-key/rotate`                                                                                                                              | Read or rotate the per-account service key bound to the tenant's API Gateway Usage Plan.                |
...
| **Maintenance access**      | `POST /accounts/{account_id}/maintenance-access`, `GET /accounts/{account_id}/maintenance-access/temp-access`, `GET /accounts/{account_id}/maintenance-access/passcode`, `GET /accounts/{account_id}/access-permission` | Mint MFA-gated temporary AWS console credentials into the account; owner-approval webhook.              |
...
9. **MFA passcodes are computed server-side from a KMS-wrapped TOTP seed.** Account does not surface the raw seed to any client; `GET /accounts/{account_id}/maintenance-access/passcode` derives the current 6-digit code from the encrypted seed in the matching `MaintenanceLoginsTable` row and returns only the code while `now < expires_at` and `iam_cleanup_at` is absent. The encrypted seed is retained only for the 24-hour audit-row window and is destroyed when the row TTLs out.
...
| **Teardown workers**        | `MarkAccountDisabling`, `DeleteRoute53Records`, `DeleteAcmCertificates`, `DeleteApiGatewayMappings`, `RevokeAllApiKeysForAccount`, `WriteDisabledAccountRow`, `EmitDeletionCallback`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 1024 MB    | 15 min   | —   | Step Functions task                                                   |
...
| **API key workers**         | `MintAccountApiKey` (UUID v4 + `apigateway:CreateApiKey` + Usage Plan binding via `ApiKeyBinder`), `RevokeApiKey` (Usage Plan unbind + `apigateway:DeleteApiKey` + row → `REVOKED`), `RotateServiceApiKey` (cross-account `sts:AssumeRole` + tenant-side `apigateway:CreateApiKey` + Usage Plan rebind + KMS-encrypt → Account row + tenant-side consumer publish, then `RevokeApiKey` of the prior key after a configurable grace), `PublishTenantServiceKeyConsumers` (updates Account-managed SSM parameters and EventBridge Connections inside the tenant account), `ApiKeyBinder` (custom-resource Lambda for Usage Plan key bindings)                                                                                                                                                  | 512 MB     | 60–300 s | —   | API Gateway / Step Functions task                                     |
| **Maintenance workers**     | `MaintenanceAccessIssuer` (cross-account `iam:CreateUser` + `iam:CreateLoginProfile` + `iam:CreateVirtualMFADevice`, KMS-wraps the TOTP seed, writes the `MaintenanceLoginsTable` row, kicks off the SES owner-approval email when configured), `MaintenanceApprovalApplier` (consumes `MaintenanceApprovalQueue`, releases the SFN task token), `TempAccessConsumer` (one-shot `GET /…/temp-access` that decrypts and burns the row), `MfaPasscodeFetcher` (computes 6-digit TOTP from the encrypted seed), `IamUserGarbageCollector` (consumes `IamGarbageCollectionQueue` + DDB-stream TTL deletes; `sts:AssumeRole`s into the tenant sub-account and deletes the user, login profile, MFA device) | 512 MB     | 60–600 s | —   | API Gateway + SQS + DDB Streams + scheduled `rate(1 hour)` self-audit |
...
| **Custom resources**        | `ApiUsagePlanCustomResource` (binds `ServiceApiKey` to the Usage Plan), `ApiGatewayLogCustomResource` (installs the access log group), `KmsKeyPolicyApplier` (sets the KMS key policy across both Lambda roles)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | 256–512 MB | 30–600 s | —   | CloudFormation custom resources                                       |
...
- **API Gateway custom domain mapping** under base path `account` against the deployer-supplied `Subdomain` (`AWS::ApiGatewayV2::ApiMapping`).
- **Step Functions** state machines (six, all `tracingEnabled: true`):
  ...
  - `MaintenanceAccessStateMachine` (`STANDARD`, total timeout 24 h) — mint IAM material in the tenant sub-account, optionally pause for owner approval (`waitForTaskToken`), surface the temp-access link, schedule the IAM user teardown (see §5.6).
- **EventBridge Scheduler** group `account-schedules` hosts a `rate(1 hour)` schedule that targets `IamUserGarbageCollector` for self-audit so any maintenance IAM user older than `MAINTENANCE_ACCESS_TTL_MINUTES` is cleaned up even if the matching DDB row is still present, stale, or missing.
...
- The **API key workers** add `apigateway:{POST,DELETE}` on `/apikeys` and `/usageplans/{id}/keys` for the marketplace API; `MintAccountApiKey` is additionally allowed `apigateway:UpdateUsagePlan`. `RotateServiceApiKey` adds `sts:AssumeRole` against the tenant `OrganizationAccountAccessRole` (the actual tenant-side `apigateway:`* calls are made via the assumed-role session, never with marketplace credentials).
- The **maintenance workers** add `sts:AssumeRole` against the tenant `OrganizationAccountAccessRole` (constrained to the configured Organizations OU IDs); the assumed session is then used to call `iam:{CreateUser,DeleteUser,CreateLoginProfile,DeleteLoginProfile,CreateAccessKey,DeleteAccessKey,CreateVirtualMFADevice,DeleteVirtualMFADevice,EnableMFADevice,DeactivateMFADevice,ListAttachedUserPolicies,AttachUserPolicy}` against `arn:aws:iam::<tenant>:user/account-maintenance/`*. The marketplace side additionally has `iam:GetAccountSummary` for the self-audit cron.
...
| `MFA_TOTP_SEED_KMS_CONTEXT`                                                         | `account/maintenance/totp`               | KMS encryption-context bound to every TOTP-seed encryption (defence in depth).                   |
...
- `otplib` for server-side TOTP generation.
...
type MaintenanceSession = {
  username: string;                          // PK; SSO identity of the requester
  ...
  mfa_device_arn?: string;                   // The virtual MFA device ARN (when user_type = MAINTENANCE)
  mfa_seed_enc?: string;                     // KMS-encrypted base32 TOTP seed (encryption-context = MFA_TOTP_SEED_KMS_CONTEXT)
...
- `GET /accounts/{account_id}/bootstrap-manifest` — API-key gated against the account owner key. Returns the non-secret bootstrap inputs for a provisioned tenant: `{ account_id, aws_account_id, subdomain, fqdn, hosted_zone_id, certificates, service_api_key_id, service_api_key_rotated_at, marketplace_api_url, account_domain_config_ssm_param, deploy_regions, system_components: { deployer: { product_id, component_id }, marketplace_puller: { product_id, component_id } } }`. It never returns plaintext API keys or AWS credentials; the bootstrap CLI obtains the tenant service-key plaintext through `GET /accounts/{account_id}/service-key` when the caller also has Comply Authentication, and obtains AWS credentials from the operator's configured AWS SSO/profile.
...
- `PUT /accounts/{account_id}/custom-domains` — body `{ domains: [{ domain_name, kind: "CANONICAL" | "ALIAS", certificate_arn?, hosted_zone_id? }], target_fqdn? }`. Upserts rows in `CustomDomainsTable` and starts `DomainConfigStateMachine` to validate / self-heal the Route 53 and ACM metadata Account owns. It does **not** create service base-path mappings. Returns `{ status: "STARTED", execution_arn }`.
- `GET /accounts/{account_id}/domain-config` — returns the standardized domain input Deployer passes to service CDK stacks: `{ domain_name, custom_domains, hosted_zone_id, certificate_arn, regional_certificates? }`. Service stacks use these values to define their own `AWS::ApiGatewayV2::ApiMapping` resources.
...
- `GET /accounts/{account_id}/maintenance-access/passcode?username=` — Comply Authentication required and `username` must equal the bearer's identity. Returns `{ passcode: "######", valid_for_seconds }` (server-side TOTP using the KMS-decrypted `mfa_seed_enc`) only while `now < expires_at` and `iam_cleanup_at` is absent. 200 / 404 / 410.
...
- **State machine** (STANDARD, total timeout 3600 s):
  ...
  AssumeTenantRole → RevokeAllApiKeysForAccount (Distributed Map over ApiKeysTable.account_id_index rows; each row → Usage Plan unbind + apigateway:DeleteApiKey + status = REVOKED)
                    → DeleteApiGatewayMappings (Distributed Map over CustomDomainsTable rows for this account)
  ...
  ```
- Account **does not** delete product CloudFormation stacks inside the tenant sub-account. Whoever installed those stacks (the deployer, plus any product self-deletion endpoints) is responsible for tearing them down before `POST /accounts/{account_id}/disable` is called. If product stacks remain when DNS deletion runs, ACM certificate deletion will fail (because they're still in use) and the teardown SFN flips the row to `ERROR`.
...
- **State machine** (STANDARD):
  ...
  ```
- The workflow validates only Account-owned DNS and certificate facts. It never calls `apigateway:CreateApiMapping`; product route mappings are created by the service CDK stacks that Deployer runs.
...
- **State machine** (STANDARD, total timeout 600 s):
  ...
  AssertNoRotationInFlight (DDB conditional update on AccountsTable row at latest version: service_api_key_rotation_in_progress = false → true)
    ...
    → MintNewServiceKey (Lambda — apigateway:CreateApiKey + Usage Plan key binding inside the tenant sub-account; writes a new ApiKeysTable row at kind = "service", ACTIVE)
    ...
    → PublishTenantServiceKeyConsumers (Lambda — using the tenant STS session, writes the current plaintext key to Account-managed SecureString parameters and updates known EventBridge Connections such as Puller's MarketplaceApiKeyConnection)
    ...
    → RevokePriorServiceKey (Lambda — Usage Plan unbind + apigateway:DeleteApiKey of the prior service key inside the tenant sub-account; flips the prior ApiKeysTable row to REVOKED)
...
- Account is the source of truth for the current valid service key. Any tenant-side storage of the plaintext is Account-managed, not product-managed: `PublishTenantServiceKeyConsumers` updates `/account/current-service-api-key` and product-specific aliases such as `/marketplace-puller/<stage>/marketplace-api-key`, then calls EventBridge `UpdateConnection` for known API destinations that hold the key (for Puller, `MarketplaceApiKeyConnection`). The old key is not revoked until these updates and the verification step succeed.
...
- **State machine** (STANDARD, total timeout = `MAINTENANCE_APPROVAL_TIMEOUT_MINUTES + MAINTENANCE_ACCESS_TTL_MINUTES + 5 min` slack):
  ```
  PurgePriorSession           (Lambda — looks up any prior MaintenanceLoginsTable row for (username, account_id),
    ...
    → MintIamUser              (iam:CreateUser at /MAINTENANCE_USERNAME_PREFIX/<username>-<short-uuid>,
                                 iam:CreateLoginProfile with random one-time password,
                                 iam:CreateVirtualMFADevice, iam:EnableMFADevice)
    → KmsWrapCredentialsAndSeed (Lambda — KMS-encrypts the {console-username, account-alias, password}
                                  AND the TOTP base32 seed under MFA_TOTP_SEED_KMS_CONTEXT;
                                  writes the MaintenanceLoginsTable row at approval_state="PENDING")
    → ChooseApprovalPath (Choice)
        ...
        └── otherwise                                               → AwaitOwnerApproval (waitForTaskToken,
                                                                       timeout MAINTENANCE_APPROVAL_TIMEOUT_MINUTES)
    ...
    SendMaintenanceLinkEmail → ScheduleSessionExpiry (waits MAINTENANCE_ACCESS_TTL_MINUTES, then enqueues IamGarbageCollectionQueue)
                              → Success
...
- `IamUserGarbageCollector` is the single chokepoint for tenant-side IAM teardown. It consumes from `IamGarbageCollectionQueue` (explicit revoke + TTL stream) and runs on a `rate(1 hour)` self-audit schedule that lists IAM users under `MAINTENANCE_USERNAME_PREFIX` whose `CreateDate < now - MAINTENANCE_ACCESS_TTL_MINUTES`. For each expired user it deletes the login profile, deactivates/deletes the MFA device, deletes the IAM user, and idempotently updates any matching `MaintenanceLoginsTable` row with `approval_state="EXPIRED"` when still non-terminal plus `iam_cleanup_at=now`. A matching DDB row is an audit record, not a deletion guard. This guarantees no orphaned IAM user lives beyond ~1 h even on stack errors.
...
- Sensitive fields are application-level encrypted via `EncryptionService.encrypt(key, plaintext)` (AES-256-GCM, key derived from `DataKey` via `kms:GenerateDataKey`):
  ...
  - `MaintenanceSession.mfa_seed_enc` — the base32 TOTP seed, decrypted on each `GET /…/passcode` call (the seed never leaves the Lambda invocation memory).
...
2. **Runtime dependencies**: `@aws-lambda-powertools/{logger,metrics,event-handler,tracer,parameters}`, `@aws-sdk/client-{dynamodb,s3,sfn,sqs,apigateway,kms,lambda,ssm,sts,secrets-manager,ses,sesv2,iam,acm,route53,organizations,events,scheduler}`, `@aws-sdk/credential-providers`, `@aws-sdk/lib-dynamodb`, `aws4`, `jose`, `otplib`, `undici`, `zod`. CDK: `aws-cdk-lib`, `constructs`, `aws-cdk`.
3. **Schemas / contracts**: one module per group in `lambda/schemas/` — `account/{create,update,confirm-email}`, `api-key/{mint,revoke,rotate-service}`, `provision/{provision,disable,certificate}`, `custom-domain`, `maintenance-access/{request,approve,temp-access,passcode}`, `http`, `errors`. Errors are tagged classes; everything else is type+validator pairs.
4. **Utils**: `lambda/utils/{encryption,hmac-fingerprint,jwt,totp,approval-signer,jsonResponse,strict-transport-security,trace,date,sigv4}.ts`.
...
- `POST /accounts/{account_id}/disable` revokes every API key bound to the account, deletes Route 53 records, ACM certificates, and API Gateway custom-domain mappings, writes a new version at `status = DISABLED`, and POSTs `/accounts/{account_id}/delete-callback` exactly once. It does **not** attempt to delete product CloudFormation stacks (out of scope).
- `PUT /accounts/{account_id}/custom-domains` is idempotent: re-running the same body does not create duplicate `AWS::ApiGatewayV2::ApiMapping` resources, and previously-failed mappings are healed by the FixFailedMappings branch of the SFN.
...
- `GET /accounts/{account_id}/maintenance-access/temp-access` is **single-use**: a second call against the same `temp_access_uuid` returns `410 MaintenanceSessionAlreadyConsumed`. The TOTP `passcode` endpoint authenticates against the AWS console for the bound IAM user.
