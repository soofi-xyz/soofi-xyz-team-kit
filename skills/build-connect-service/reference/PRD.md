...
| **Vendor request workers** (one per `Type` in §3.5.1)                                          | `HttpJsonWorker`, `HttpXmlWorker`, `HttpAnyWorker`, `GetJsonWorker`, `PostJsonWorker`, `PutJsonWorker`, `DeleteJsonWorker`, `GetXmlWorker`, `PostXmlWorker`, `DeleteXmlWorker`, `PostMultipartWorker`, `PutMultipartWorker`, `PostBlobWorker`, `PutBlobWorker`, `PostBase64Worker`, `PostFormUrlEncodedWorker`, `GetFileWorker`                                                                                                                               | 1024 MB    | 20–120 s | —                                                                              | Step Functions task in compiled flow                                                                                          |
| **Static-IP variants**                                                                         | `HttpJsonStaticIpWorker`, `HttpXmlStaticIpWorker`                                                                                                                                                                                                                                                                                                                                                                                                             | 1024 MB    | 120 s    | ✓ (`LambdaSg` in two private subnets so egress goes through the on-demand NAT) | Step Functions task                                                                                                           |
...
- **Step Functions** state machines (six, all `tracingEnabled: true`):
  ...
  The **runtime** state machines created by `CreateRuntimeStateMachine` / `UpdateRuntimeStateMachine` are dynamic, named `<product?>-<vendor>-<flow>[-<token>]`, and use a constrained `RuntimeStateMachineRole` (only `lambda:InvokeFunction` on worker ARNs — including `SftpPoller` — and `sqs:SendMessage` on `WebhooksQueue`).
...
- `@aws-sdk/client-{dynamodb,s3,sfn,sqs,apigateway,ec2,kms,lambda,ssm,sts,transfer,secrets-manager,scheduler,eventbridge}` and `@aws-sdk/credential-providers` for AWS calls.
...
The compiler (`lambda/services/flow-compiler/transformation.ts`) supports the following `ResourceDefinition.Type` values; each maps to a single worker Lambda whose ARN is read from env vars:


| Type                                                      | Worker                                                 | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| --------------------------------------------------------- | ------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `HTTP_JSON` (+ `HTTP_JSON_WITH_EIP`)                      | `HttpJsonWorker` / `HttpJsonStaticIpWorker`            | Generic JSON HTTP call. EIP variant is VPC-bound, used when the partner allow-lists the static IP.                                                                                                                                                                                                                                                                                                                                                                                        |
| `HTTP_XML` (+ `HTTP_XML_WITH_EIP`)                        | `HttpXmlWorker` / `HttpXmlStaticIpWorker`              | Generic XML HTTP call (with `fast-xml-parser` XML→JSON pre-flatten).                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `HTTP_ANY`                                                | `HttpAnyWorker`                                        | Generic transport-agnostic HTTP.                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `GET_JSON`, `POST_JSON`, `PUT_JSON`, `DELETE_JSON`        | `Get/Post/Put/DeleteJsonWorker`                        | Verb-specialised JSON convenience tasks.                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `GET_XML`, `POST_XML`, `DELETE_XML`                       | `Get/Post/DeleteXmlWorker`                             | Verb-specialised XML tasks.                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `POST_MULTIPART`, `PUT_MULTIPART`                         | `Post/PutMultipartWorker`                              | Multipart upload from spec or from a presigned source.                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `POST_BLOB`, `PUT_BLOB`                                   | `Post/PutBlobWorker`                                   | Form-data variants accepting a single file via `blob_url` (streamed via `undici` `Readable.from(response.body)`).                                                                                                                                                                                                                                                                                                                                                                         |
| `POST_BASE64`, `POST_FORM_URL_ENCODED`                    | `PostBase64Worker`, `PostFormUrlEncodedWorker`         | Helpers for partners that require encoded payloads.                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `GET_FILE`                                                | `GetFileWorker`                                        | Stream a partner file → S3 → mint a presigned URL.                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
- `GET /vendors/{vendor_name}/information` — returns `{ vendor_name, product_name?, credentials: { credentials: bool, "passed-credentials": bool } }` (presence-only; never the values).
- `PATCH /vendors/{vendor_name}/pulled-credentials` — synchronously SigV4-fetches credentials (via `aws4`) from the upstream `customer-account-manager/retrieve-credentials` endpoint and merges them in (`type=passed_credentials` rows route into `passed_credentials`, all other rows into `credentials`). 422 on Account Manager failure.
...
- Outbound HTTP uses `**undici`** with a process-wide `Agent` (`new Agent({ keepAliveTimeout: 60_000, keepAliveMaxTimeout: 600_000 })`). For SigV4-signed cross-service calls, `aws4` signs the headers before they hit `undici`.
