...
small overlay of counties published outside the catalog (currently `santa-clara`, `clay`, `lake`, and `volusia`, in
...
This writes `PROPERTY_QUERY_TABLE_MAP`, `PERMIT_QUERY_TABLE_MAP`, `DATASET_COVERAGE_MAP`, and
`PUBLISHED_COUNTY_CATALOG_URL` straight into `mcp.json`'s `mcpServers.elephant.env`, merging in
overlay counties automatically — no hand-pasting or manual overlay preservation required. Every
other env key (`ORACLE_OPEN_DATA_*`, `ORACLE_GEO_INDEX_IPNS`) and the launcher are left untouched.
Running it again after no catalog/overlay change leaves no further diff.
...
| `PROPERTY_QUERY_TABLE_MAP` | `queryProperties`, `getPropertyQuerySchema`, geo tools (SQL over open Parquet) | Bundled — every county in Oracle's published catalog, plus **santa-clara**, **clay**, **lake**, and **volusia** |
...
| MCP log shows an older `node` than your shell's (e.g. `Node.js v22.12.0` while `node -v` says 24) | A GUI-launched Cursor uses the login PATH, not your interactive nvm shell. The bundled launcher prepends the newest nvm bin that is ≥22.18 **after** `/opt/homebrew/bin:/usr/local/bin`, so a stale `/usr/local/bin/node` no longer wins; it now exits with a clear "node … is too old" message instead of a confusing module error |
| `ERR_MODULE_NOT_FOUND` (e.g. `Cannot find package 'uint8arrays'`) or `ENOTEMPTY` from `~/.npm/_npx/<hash>` | The `npx` cache was left half-installed (usually by killing the first cold start). `rm -rf ~/.npm/_npx/<hash>`, then let the next start finish uninterrupted — a cold GitHub install and build can take several minutes |
| `listPublishedCounties` returns 11 counties / no Seminole | That server is still on elephant-mcp’s default `oracle-node` GitHub catalog (or a user `~/.cursor/mcp.json` with no `PUBLISHED_COUNTY_CATALOG_URL`). Reload after `scripts/local-cursor-plugin.sh install`, and disable a competing user-level `elephant` server. See `docs/elephant-source-repos.md`. |
