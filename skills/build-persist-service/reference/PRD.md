...
- Pipeline:
  ...
  3. **Normalise for persist** — strip meta props, hash IDs, build a response copy, then **stamp** `created_at` (`g:Timestamp`, ingest-time epoch ms) on every payload vertex (`label="created_at"`, deterministic VP id `<vertexId>#created_at`) and edge. Refs are not upserted; they only populate the endpoint ID map for edge hashing and writes.
...
- **GraphSON router** depends on: Gremlin client + transactions, Lexicon loader, GraphSON semantic validator, GraphSON validator, vertex-ref verifier, GraphSON sync ingest service, GraphSON async ingest service.
...
- After validation it counts `vertices + edges` and routes with `routeForCounts`:
  - **Sync path** (`count <= SYNC_INGEST_MAX_ELEMENTS`, default 50): verify any `vertexRefs`, hash IDs, and upsert in a single Neptune writer transaction through the same `persistSync` path as `POST /persist/ingest`.
  - **Async path** (`count > SYNC_INGEST_MAX_ELEMENTS` and no refs): store the validated payload in `IngestAsyncPayloadBucket` and send a `schemaVersion="2"` message to `IngestAsyncQueue`, sharing the Stage-1/Stage-2 bulk ingest machinery with HTTP async ingest.
