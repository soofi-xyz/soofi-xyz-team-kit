...
- Marketplace does **not** itself deploy CloudFormation / CDK stacks into subscriber accounts. Subscribers receive a notification with a presigned `bundle_url` and are responsible for handing the artifact to their own deployer / puller. There is no per-subscription deploy state machine, no `WAIT_FOR_TASK_TOKEN` deployer callback, and no per-environment `environment_token` exchange.
- Marketplace does **not** maintain an `(api_key, domain_name)` trust relationship with subscribers. A subscription is identified by an opaque `subscription_id` and authenticated by knowledge of its `signing_secret` (returned once at create-time) and the API key of the marketplace tenant that created it. There is no env-admin handshake on the subscription path.
...
- Marketplace does **not** invent its own auth identity. API keys are minted upstream and bound to API Gateway usage plans by the deployer (see §2.3.2 — `ApiUsagePlanCustom`).
...
| `OntologyTable`      | `pk` (S) / `sk` (S) | `gsi1: gsi1pk`, `gsi2: gsi2pk/gsi2sk`, `gsi3: gsi3pk` | Single-table store for the catalog: families, categories, products, APIs, prices (with revisions), and configurations (per-product, per-company, with optional remote-MP cross-references). Item shape mirrors §6.3.                                                                                             |
...
- **API Gateway custom domain mapping** under base path `marketplace` against the deployer-supplied `Subdomain` (`AWS::ApiGatewayV2::ApiMapping`).
...
- Each public router holds only the DynamoDB / KMS / S3 / Step Functions permissions required by **its** routes (e.g. the catalog router has no `s3:PutObject`; the subscriptions router has no `states:StartExecution` for the publication state machine).
...
| `SERVICE_API_KEY_ID`                                          | Deployer-supplied parameter                             | `ApiUsagePlanCustom`                                                 |
...
Bundles are caller-supplied via `bundle_url` (any HTTPS URL whose `HEAD` succeeds with valid metadata in headers). After the publication state machine succeeds, Marketplace **re-uploads** the artifact to `s3://<BundlesBucket>/bundles/<product_id>/<component_id>/<bundle_id>` with a **sanitised** metadata block (status / version / applied_rules stripped from each `service-*` sub-block) and stamps `service-marketplace.id = <bundle_id>` so subscribers can match by it. `dependency_layers` is the topologically-sorted closure computed at upload time and is the same data the notification payload's `dependency_layers` field carries verbatim.
...
Family (family_name unique, family_id minted)
  └── Category (category_name unique under family)
        └── Product (product_name unique under category, product_id minted)
              ...
              ├── Configuration (per company_ids[], optional remote-MP cross-reference)
...
**Remote-MP references.** Configurations may reference a product that lives in a *different* Marketplace instance via the `{ environment: { host, api_key }, value }` shape on `configured_product_id` and `company_ids`. Marketplace validates the remote ontology by `GET https://<remote-host>/marketplace/ontology/products/<id>/(metadata|api/<api_id>)` before persisting the configuration; failures map to 400/404 with `Could not validate external Marketplace entities…`.
...
The receiver MUST respond `200 OK` or `204 No Content` and echo the identical `X-Marketplace-Hook-Secret` header. Marketplace refuses with 422 if the URL is unreachable, returns non-2xx, omits the echoed header, or echoes a different value. The handshake is not a Marketplace event: it carries no `X-Marketplace-Event`, no notification body, and no `X-Marketplace-Signature`. Subscribers can re-run the probe later via `POST /subscriptions/verify { webhook_url, signing_secret? }` (idempotent) without persisting a row; Marketplace sends the same handshake headers, using the provided secret or a one-shot probe secret if absent.
...
| 403  | `SubscriptionOwnerMismatch` (caller's API key did not create this subscription)                                                                                                              |
...
| 422  | `InvalidBundleURL`, `BundleAssessmentVersionHashOutdated`, `BundleTesterVersionHashOutdated`, `BundleTypeMismatch`, `UnknownBundleType`, `InvalidDependencySpecification`, `IncompatibleLexiconVersionId`, `WebhookUrlUnreachable`, `WebhookUrlInsecure`, `EnvAdminNotConfigured`, `RegulatorProductInvalid`, `EnvironmentInactive` |
...
6. **Bundle metadata must be decodable** — extract from response headers, JSON-decode (422 `Unknown bundle type.` on failure).
7. **Bundle's declared `bundle_type` must match the component's `type`** (422 `Invalid bundle type. Expected - <component.type>, given - <bundle_type>`).
8. **Assessment version hash must be the latest** — validate against the catalog product's regulator (422). Direct dependencies are similarly checked against the **latest or second-latest** assessment hash.
9. **DATA bundles must additionally pass the Tester version hash check** when a `service-test` block exists in the latest Test bundle's metadata.
10. **DATA components cannot have `deploy_time_dependencies`** (422).
11. **No self-dependency** (422 `cannot depend on itself`).
12. **No intersection between `dependencies` and `deploy_time_dependencies`** (422 with the intersecting names).
13. **No dependency cycle** — topological-sort dry-run over the current component plus the resolved direct dependency owners (422 `Cycle: A -> B -> A`). The resolved layers are persisted on the bundle row as `dependency_layers` and re-emitted in every subsequent notification.
14. **Every direct dependency must be a registered component with at least one bundle** (422).
15. **Lexicon-version compatibility** — when the bundle declares `service-builder.lexicon_version_id`, every dependent component must already be on a lexicon ≥ that ID; otherwise 422 `… is incompatible with its clients.`.
...
| Verb     | Path                                                                | Auth     | Behaviour                                                                                                                                                                                                                                                                                                                                                              |
| -------- | ------------------------------------------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `POST`   | `/subscriptions`                                                    | API key  | Create a subscription. Body: `{ component_id, webhook_url, description?, metadata? }`. Validates that the component exists, that the URL is `https://` and completes the `X-Marketplace-Hook-Secret` handshake, and that no `(component_id, webhook_url)` pair already exists for the caller's API key. Mints a `subscription_id` and a one-time `signing_secret`. Returns `201` with the secret in the body. |
| `GET`    | `/subscriptions`                                                    | API key  | Paginated list of subscriptions owned by the caller's API key. Supports `?component_id=<id>` filter, `?status=ACTIVE|PAUSED` filter, and `?limit=<1..100>&next_token=<base64>` pagination.                                                                                                                                                                                  |
| `GET`    | `/subscriptions/{subscription_id}`                                  | API key  | Detail of one subscription (200) or 404. `403` if the subscription belongs to a different API-key owner.                                                                                                                                                                                                                                                                |
| `PATCH`  | `/subscriptions/{subscription_id}`                                  | API key  | Update one or more of `webhook_url`, `description`, `metadata`, `status` (`ACTIVE|PAUSED`). Re-runs the `X-Marketplace-Hook-Secret` handshake when `webhook_url` changes; refuses 422 if unreachable or not echoed.                                                                                                                                                    |
| `DELETE` | `/subscriptions/{subscription_id}`                                  | API key  | Delete the subscription, its Secrets Manager secret, and (TTL'd) its notification history. `204 No Content`.                                                                                                                                                                                                                                                            |
| `POST`   | `/subscriptions/{subscription_id}/rotate-secret`                    | API key  | Mint a new `signing_secret`, atomically replace the Secrets Manager value, and return the new secret in the response body (one-time). The old secret remains valid for an overlap window of 60 s so in-flight notifications can still be verified.                                                                                                                       |
| `GET`    | `/subscriptions/{subscription_id}/notifications`                    | API key  | Paginated notification history for this subscription (sort: `desc` by `event_time`, limit ≤50). Each row carries `notification_id`, `bundle_id`, `status`, `attempt_count`, `last_attempt_at`, `last_response_status`, `last_response_excerpt` (≤512 chars), `next_attempt_at?`.                                                                                                |
| `GET`    | `/subscriptions/{subscription_id}/notifications/{notification_id}`  | API key  | Detail of one notification (200) or 404.                                                                                                                                                                                                                                                                                                                                |
| `POST`   | `/subscriptions/{subscription_id}/notifications/replay`             | API key  | Re-send the latest notification for this subscription (does NOT mint a new `event_id` — the original idempotency key is preserved so receivers can dedupe). Returns `202 { notification_id, replayed_at }`. 409 if the subscription is `PAUSED`.                                                                                                                         |
...
| Configurations | `POST`   | `/ontology/products/{product_id}/configurations`                    | Create a configuration. Body: `{ configuration_description, configured_product_id, configured_product_api_id?, company_ids[] }`. `configured_product_id` and `company_ids` may be remote-MP references (see §3.4).                   |
...
|                | `PATCH`  | `/ontology/products/{product_id}/configurations/{configuration_id}` | Extend (companies / description / configured_product_id / configured_product_api_id). Cross-environment company hosts must match the existing configuration's `company_host`.                                                        |
...
| Verb  | Path               | Auth    | Behaviour                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ----- | ------------------ | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PUT` | `/settings`        | API key | Replace one or more instance settings (review/self-cleaning-review/site/quicksight/env-admin/regulator). Each non-admin key is validated by calling env-admin to ensure the corresponding environment is reachable and not in a `BAD_ENVIRONMENT_STATUSES` state. The `code_assessment_regulator.regulator_product_identifier` is validated by hitting `GET https://<host>/marketplace/ontology/products/<id>` (422 if not found). Persists each value to its `*_SSM_PARAM` (KMS-encrypted SecureString). |
...
- **Inputs**: `{ bundle_url, product_id, component_id, type, bundle_id, dependencies[], deploy_time_dependencies[], dependency_layers, decoded_bundle_metadata, host, api_key, previous_bundle_id?, review_environments, skip_review }`.
...
- **Sandbox delivery** (`DeployBundleToSandbox`): pushes the bundle into the configured `review_api_key` env (and the optional `self_cleaning_review_api_key` env in parallel) to confirm the artifact actually deploys end-to-end. On terminal failure the bundle is marked `Failed` and never fans out to subscribers.
- **Re-upload + sanitisation** (`PutBundleToS3`): re-checks the bundle URL availability, sanitises the metadata (strip `service-*.{status, applied_rules, version}`), stamps `service-marketplace = { component_id, id: bundle_id, status: "SUCCEEDED", issuer: <host>marketplace/, timestamp, version: "1.0" }`, uploads to `s3://<BundlesBucket>/bundles/<product_id>/<component_id>/<bundle_id>` with metadata header, and updates the DDB row to `in_review=true, decoded_bundle_metadata=<full>` so the next `UpdateBundleStatus` step can flip `bundle_status=Valid`.
...
{ pk: "COMPONENT#<component_id>",
  ...
  owner_api_key_id,                             // API Gateway api_key.id of the creator (for ownership checks)
  status: "ACTIVE|PAUSED",
...
{ pk: "CONFIG",   sk: "<configuration_id>", configuration_id, product_id, configuration_description,
  configured_product_id, configured_product_api_id?, configured_product_host?, company_ids[],
  company_host?, gsi2pk: "P#<product_id>", gsi3pk: "COMPANY#<company_id>" (one row per company) }
```
...
- **API key** (`x-api-key`) — every public route except `GET /information`. The owning API key of a subscription (the one that created it) is recorded as `owner_api_key_id` on the subscription row; mutating routes (`PATCH`, `DELETE`, `rotate-secret`, `replay`) refuse 403 if the caller's API key id does not match.
...
- A deployer-supplied `ServiceApiKeyID` (the canonical operator API key).
- A configured **review environment** reachable by `review_api_key` (Marketplace will only fan out bundles that pass the sandbox review).
...
1. `PUT /settings` with at least `review_api_key` (so bundle uploads can run a sandbox review). Additional keys (`self_cleaning_review_api_key`, `site_api_key`, `quicksight_api_key`, `env_administrator_*`, `code_assessment_regulator`) are optional but unlock further capabilities (parallel review env, site publication, analytics dump, regulator-product-bound assessment validation).
...
9. **Custom resources**: `lambda/custom-resources/{api-usage-plan,api-gateway-log,managed-ontology-v2}.ts` plus the `cfnresponse`-equivalent helper.
...
- Subscription mutating routes (`PATCH`, `DELETE`, `rotate-secret`, `replay`) refuse `403 SubscriptionOwnerMismatch` when the caller's API key id is not the subscription's `owner_api_key_id`.
